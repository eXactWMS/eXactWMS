...
begin
  ...
  if PgcBase.ActivePage = TabRelVolumes then Begin//Analitico
     if PgcVolumesGeral.ActivePage = TabVolumeResumo then Begin
        if (Not FDMemVolumeConsulta.Active) or (FDMemVolumeConsulta.IsEmpty) then
           raise Exception.Create('Não há dados de Volume(s) para exportar!');
        if (BtnExportarStand.Grayed) then Exit;
        Try
          ExportarExcel(FDMemVolumeConsulta);
        Except
...
//  if LstPedidosAdv.ColCount = 27 then
     LstPedidosAdv.ColWidths[26] := 250+Trunc(250*ResponsivoVideo);
...
{  LstVolumeHistoricoConsulta.Left := LstVolumeConsulta.Left+
                                     LstVolumeConsulta.ColWidths[0]+LstVolumeConsulta.ColWidths[1]+LstVolumeConsulta.ColWidths[2]+
                                     LstVolumeConsulta.ColWidths[3]+LstVolumeConsulta.ColWidths[4]+LstVolumeConsulta.ColWidths[5]+
                                     LstVolumeConsulta.ColWidths[6]+LstVolumeConsulta.ColWidths[7]+LstVolumeConsulta.ColWidths[8]+
                                     LstVolumeConsulta.ColWidths[9]+LstVolumeConsulta.ColWidths[10]+30;
...
//    LstPedidosAdv.AddDataImage(22, xPed, 2, TCellHAlign.haCenter, TCellVAlign.vaTop);
  for xPed := 0 to Pred(pJsonArray.Count) do Begin
    ...
    LstPedidosAdv.Cells[26, xPed+1]    := pJsonArray.Get(xPed).GetValue<String>('motivoexclusao');
...
begin
  inherited;
  ImprimirExportar(LstVolumeConsulta.RowCount>1);
  LstVolumeHistoricoConsulta.Left := LstVolumeConsulta.Left + LstVolumeConsulta.Width+30;
end;
