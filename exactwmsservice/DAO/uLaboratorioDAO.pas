...
Begin
  ...
  Begin
    Try
      ...
      FConexao.Query.SQL.Clear;
      FConexao.Query.SQL.Add(tuEvolutConst.SqlInsFabricante);
      FConexao.Query.ParamByName('pCodERP').Value   := pJsonArray.Items[xFabr].GetValue<Integer>('fabricanteid');
      FConexao.Query.ParamByName('pNome').Value     := pJsonArray.Items[xFabr].GetValue<String>('nome');
      FConexao.Query.ParamByName('pFone').Value     := pJsonArray.Items[xFabr].GetValue<String>('fone');
      FConexao.Query.ParamByName('pEmail').Value    := pJsonArray.Items[xFabr].GetValue<String>('email');
      FConexao.Query.ParamByName('pHomepage').Value := pJsonArray.Items[xFabr].GetValue<String>('homepage');
      FConexao.Query.ParamByName('pStatus').Value   := pJsonArray.Items[xFabr].GetValue<Integer>('status', 1);
      if DebugHook <> 0 then
        FConexao.Query.SQL.SaveToFile('LaboratorioImporta.Sql');
      FConexao.Query.ExecSQL;
      Result.AddElement(TJsonObject.Create.AddPair('Ok',
