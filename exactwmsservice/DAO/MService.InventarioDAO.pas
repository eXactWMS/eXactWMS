...
begin
  Try
    ...
    End;
  Finally
    QyrInventarioModelo.Close;
    QryInventarioNovo.Close;
    QryEnderecoModelo.Close;
  End;
end;
...
begin
  ...
  Begin
    Try
      FConexao.Query.Close;
      FConexao.Query.SQL.Clear;
      FConexao.Query.SQL.Add('Select ProcessoId From ProcessoEtapas Where Descricao = ' + #39 + 'Inventario - Em Contagem' + #39);
      FConexao.Query.Open;
...
begin
  vParamOk := 0;
  Try
    FConexao.Query.SQL.Add(TuEvolutConst.SqlGetRelatorioAjuste);
    ...
    if AParams.ContainsKey('datacriacaoinicial') then begin
       FConexao.Query.ParamByName('pDataCriacaoinicial').Value := AParams.Items['datacriacaoinicial'];
       Inc(vParamOk);
    end
    Else FConexao.Query.ParamByName('pdatacriacaoinicial').Value := 0;
    ...
    if AParams.ContainsKey('datafinalizacaoinicial') then begin
       FConexao.Query.ParamByName('pDataFinalizacaoInicial').Value := AParams.Items['datafinalizacaoinicial'];
       Inc(vParamOk);
    end
    Else
       FConexao.Query.ParamByName('pDataFinalizacaoInicial').Value := 0;
    if AParams.ContainsKey('datafinalizacaofinal') then begin
       FConexao.Query.ParamByName('pDataFinalizacaoFinal').Value := AParams.Items['datafinalizacaoFinal'];
       Inc(vParamOk);
    end
    Else
       FConexao.Query.ParamByName('pDataFinalizacaoFinal').Value := 0;
    If DebugHook <> 0 then
       FConexao.Query.SQL.SaveToFile('RelatorioAjuste.Sql');
    FConexao.Query.Open();
...
begin
  ...
  try
    ...
    If DebugHook <> 0 Then
      FConexao.Query.SQL.SaveToFile('Inventario_InsertUpdate.Sql');
    // FConexao.Query.ExecSQL;
    FConexao.Query.Open;
