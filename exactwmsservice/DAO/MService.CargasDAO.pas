...
function TCargasDAO.GetCargaPedidoVolumesConferencia
(pCargaId: Integer; pProcesso: String) : TJSonArray;
begin
  FConexao.Query.SQL.Add(TuEvolutConst.SqlGetCargaPedidoVolumesConferencia);
  FConexao.Query.ParamByName('pCargaId').Value := pCargaId;
  FConexao.Query.ParamByName('pProcesso').Value := pProcesso;
  if DebugHook <> 0 then
    FConexao.Query.SQL.SaveToFile('CargaPedidoVolumes.Sql');
...
begin
  ...
  Else
     FConexao.Query.ParamByName('pprocessoid').Value := 0;
  if DebugHook <> 0 then
     FConexao.Query.SQL.SaveToFile('RelPedidoParaCargas.Sql');
  FConexao.Query.Open();
  if FConexao.Query.IsEmpty then Begin
     Result := TJSonArray.Create();
     Result.AddElement(TJsonObject.Create.AddPair('Erro', TuEvolutConst.QrySemDados));
...
begin
  try
    ...
    FConexao.Query.ParamByName('pProcessoId').Value := pJsonObjectCargas.GetValue<Integer>('processoid');
    FConexao.Query.ParamByName('pUsuarioId').Value := pJsonObjectCargas.GetValue<Integer>('usuarioid');
    FConexao.Query.ParamByName('pTerminal').Value := pJsonObjectCargas.GetValue<String>('terminal');
    if DebugHook <> 0 then
    ...
    if vListaCargaOff <> '' then Begin
       ...
       FConexao.Query.SQL.Add('From DocumentoEtapas De');
       FConexao.Query.SQL.Add('Inner Join Cargas C On C.Uuid = De.Documento');
       FConexao.Query.SQL.Add('Where C.CargaId In ('+vListaCargaOff+') and De.ProcessoId = '+pJsonObjectCargas.GetValue<String>('processoid'));
       FConexao.Query.ExecSQL;
    End;
    Result := TJSonArray.Create;
