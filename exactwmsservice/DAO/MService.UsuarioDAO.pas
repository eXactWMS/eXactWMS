...
Begin
  try
    ...
    If Debughook <> 0 then
       FConexao.Query.SQL.SaveToFile('AcessoFuncionalidade.Sql');
    FConexao.Query.Open;
...
begin
  try
    FConexao.Query.Sql.Add('select U.UsuarioId, U.Login, U.Nome, U.Senha, U.SenhaPadrao, U.DiasSenhaValida, U.Senha, U.Email, ');
    FConexao.Query.Sql.Add('       P.PerfilId, P.Descricao, P.Status StatusPerfil, FORMAT(Dp.Data, '+#39+'dd/MM/yyyy'+#39+') Data,');
    FConexao.Query.Sql.Add('       CONVERT(Time(0), Hp.hora, 0) hora, FORMAT(UAS.Data, ' + #39 + 'dd/MM/yyyy' + #39+') as DtUltimaAlteracaoSenha,');
    FConexao.Query.Sql.Add('       CONVERT(VARCHAR(5), HAS.hora,108) as HrUltimaAlteracaoSenha, FORMAT(DI.Data, '+#39+'dd/MM/yyyy'+#39+') as DtInclusao,');
    FConexao.Query.Sql.Add('       CONVERT(VARCHAR(5), HI.hora,108) as HrInclusao, U.Status, Coalesce(UL.IdLogOn, 0) IdLogOn, ');
    FConexao.Query.Sql.Add('       Ul.Terminal, Ul.DataInicio, Ul.HoraInicio');
    FConexao.Query.Sql.Add('From Usuarios U ');
    FConexao.Query.Sql.Add('Inner Join Perfil P On P.PerfilId = U.PerfilId');
    FConexao.Query.Sql.Add('Left Join Rhema_Data UAS ON UAS.IdData = U.DtUltimaAlteracaoSenha ');
    FConexao.Query.Sql.Add('Join Rhema_Hora HAS ON HAS.IdHora = U.HrUltimaAlteracaoSenha');
    FConexao.Query.Sql.Add('Left Join Rhema_Data DI ON DI.IdData = U.DtInclusao ');
    FConexao.Query.Sql.Add('Left Join Rhema_Hora HI ON HI.IdHora = U.HrInclusao');
    FConexao.Query.Sql.Add('Left Join Rhema_Data DP ON DP.IdData = P.Data ');
    FConexao.Query.Sql.Add('Left Join Rhema_Hora HP ON HP.IdHora = P.Hora');
    FConexao.Query.Sql.Add('Left Join (Select Top 1 UsuarioId, Coalesce(IdLogOn, 0) IdLogOn, Terminal, DataInicio, HoraInicio');
    FConexao.Query.Sql.Add('           From UsuarioLogOnOff');
    FConexao.Query.Sql.Add('           Where OnOff = 1) Ul On Ul.UsuarioId = U.UsuarioId');
    FConexao.Query.Sql.Add('where U.Status = 1');
    FConexao.Query.Sql.Add('Order by U.Nome');
    If DebugHook <> 0 Then
       FConexao.Query.Sql.SaveToFile('GetUsuarioAtivo.Sql');
    FConexao.Query.Open;
    if FConexao.Query.IsEmpty then Begin
       Result := TjSonArray.Create;
  ...
  try
    if pUsuarioId = 0 then
      vSql := 'Insert Into Usuarios (Login, Nome, Senha, SenhaPadrao, DiasSenhaValida, Email, PerfilId, '+
        'DtUltimaAlteracaoSenha, HrUltimaAlteracaoSenha, DtInclusao, HrInclusao, Status) '+
    ...
    FConexao.Query.Open;
    vUsuarioId := FConexao.Query.FieldByName('UsuarioId').AsInteger;
    Result := TJSONArray.Create;
    Result.AddElement(TJsonObject.Create.AddPair('usuarioid', FConexao.Query.FieldByName('UsuarioId').AsInteger));
  Except On E: Exception do Begin
      raise Exception.Create(E.Message);
    End;
  end;
