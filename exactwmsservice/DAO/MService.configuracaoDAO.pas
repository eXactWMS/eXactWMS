...
uses
  ...
  exactwmsservice.dao.base, exactwmsservice.lib.utils;
...
begin
  try
    vSql := 'select Cfg.*, TEnd.Descricao EnderecoVolumeExpedidoCancelado, (select Producao From EstoqueTipo Where id = 1) BuscarStage' +
...
begin
  try
    ...
    FConexao.Query.Sql.Add('Update configuracao ' + sLineBreak +
      ...
      //Produtividade
      FConexao.Query.Sql.Add('     , ArmazenagemMeta               = ' + pJsonConfiguracao.GetValue<Integer>('armazenagemMeta').Tostring);
      FConexao.Query.Sql.Add('     , ArmazenagemTolerancia         = ' + pJsonConfiguracao.GetValue<Integer>('armazenagemTolerancia').Tostring);
      FConexao.Query.Sql.Add('     , SeparacaoFracionadoMeta       = ' + pJsonConfiguracao.GetValue<Integer>('separacaoFracionadoMeta').Tostring);
      FConexao.Query.Sql.Add('     , SeparacaoFracionadoTolerancia = ' + pJsonConfiguracao.GetValue<Integer>('separacaoFracionadoTolerancia').ToString);
      FConexao.Query.Sql.Add('     , CheckoutFracionadoMeta        = ' + pJsonConfiguracao.GetValue<Integer>('checkoutFracionadoMeta').ToString);
      FConexao.Query.Sql.Add('     , CheckoutFracionadoTolerancia  = ' + pJsonConfiguracao.GetValue<Integer>('checkoutFracionadoTolerancia').ToString);
      FConexao.Query.Sql.Add('     , CheckoutCxaFechadaMeta        = ' + pJsonConfiguracao.GetValue<Integer>('checkoutCxaFechadaMeta').ToString);
      FConexao.Query.Sql.Add('     , CheckoutCxaFechadaTolerancia  = ' + pJsonConfiguracao.GetValue<Integer>('checkoutCxaFechadaTolerancia').ToString);
      FConexao.Query.Sql.Add('     , ExpedicaoMeta                 = ' + pJsonConfiguracao.GetValue<Integer>('expedicaoMeta').ToString);
      FConexao.Query.Sql.Add('     , ExpedicaoTolerancia           = ' + pJsonConfiguracao.GetValue<Integer>('expedicaoTolerancia').ToString);
      FConexao.Query.Sql.Add('     , ReposicaoColetaMeta           = ' + pJsonConfiguracao.GetValue<Integer>('reposicaoColetaMeta').ToString);
      FConexao.Query.Sql.Add('     , ReposicaoColetaTolerancia     = ' + pJsonConfiguracao.GetValue<Integer>('reposicaoColetaTolerancia').ToString);
      FConexao.Query.Sql.Add('     , CheckInMeta                   = ' + pJsonConfiguracao.GetValue<Integer>('checkInMeta').ToString);
      FConexao.Query.Sql.Add('     , CheckInTolerancia             = ' + pJsonConfiguracao.GetValue<Integer>('checkInTolerancia').ToString);
      FConexao.Query.Sql.Add('');
      FConexao.Query.Sql.Add('Update EstoqueTipo Set Producao = '+pJsonConfiguracao.GetValue<Integer>('buscarStage').ToString+' Where Id = 1');
    If DebugHook <> 0 then
...
var
  ...
  FConexaoJsonIntegracao : TConnection;
  BancoDados : String;
begin
  ...
  try
    ...
    try
      ...
      FconexaoLog.Query.Sql.Add('IF EXISTS(SELECT top 1 ' + #39 + 'x' + #39 +
        ' FROM RequestResponse WHERE data <= GetDate()-15  )');
      If Not FConexaoLog.DB.Connected then
         FConexaoLog.DB.Connected := True;
      FconexaoLog.Query.Sql.Add('    goto deleteMore');
      If DebugHook <> 0 then
         FConexaoLog.Query.SQL.SaveToFile('ManutencaoLog.Sql');
      FconexaoLog.Query.ExecSql;
      Result := TJsonObject.Create.AddPair('Ok', 'Log(s) apagado(s).');

//expurgo de dados - JsonIntegracao
      FConexaoJsonIntegracao := TConnection.Create(0);
      BancoDados := FConexaoJsonIntegracao.DB.Params.Database;
      FConexaoJsonIntegracao.Query.Sql.Add('BACKUP LOG '+BancoDados+' TO DISK = '+#39+'NUL'+#39);
      FConexaoJsonIntegracao.Query.Sql.Add('declare @id Integer = 0 ');
      FConexaoJsonIntegracao.Query.Sql.Add('set  @id =  (Select Max(Id) From JsonIntegracao)-2000');
      FConexaoJsonIntegracao.Query.Sql.Add('');
      FConexaoJsonIntegracao.Query.Sql.Add('-- contador de execuções');
      FConexaoJsonIntegracao.Query.Sql.Add('declare @contador int');
      FConexaoJsonIntegracao.Query.Sql.Add('set @contador = 0');
      FConexaoJsonIntegracao.Query.Sql.Add('');
      FConexaoJsonIntegracao.Query.Sql.Add('-- laço com a ação desejada');
      FConexaoJsonIntegracao.Query.Sql.Add('deleteMore:');
      FConexaoJsonIntegracao.Query.Sql.Add('Begin');
      FConexaoJsonIntegracao.Query.Sql.Add('  DELETE TOP(10000)  FROM JsonIntegracao WHERE Id < @id');
      FConexaoJsonIntegracao.Query.Sql.Add('  CHECKPOINT;  -- Força a gravação dos dados em disco para evitar inchaço do log');
      FConexaoJsonIntegracao.Query.Sql.Add('  WAITFOR DELAY '+#39+'00:00:01'+#39+'; -- Pequena pausa para evitar travamentos');
      FConexaoJsonIntegracao.Query.Sql.Add('  set @contador = @contador + 1');
      FConexaoJsonIntegracao.Query.Sql.Add('  --print '+#39+'rodada '+#39+' + cast(@contador as varchar(10)) + '+#39+'- 10000 registros excluídos'+#39);
      FConexaoJsonIntegracao.Query.Sql.Add('End');
      FConexaoJsonIntegracao.Query.Sql.Add('');
      FConexaoJsonIntegracao.Query.Sql.Add('IF EXISTS(SELECT top 1 '+#39+'x'+#39+' FROM JsonIntegracao WHERE Id < @id )');
      FConexaoJsonIntegracao.Query.Sql.Add('    goto deleteMore');
      FConexaoJsonIntegracao.Query.ExecSql;
    except
      On E: Exception Do
      Begin
