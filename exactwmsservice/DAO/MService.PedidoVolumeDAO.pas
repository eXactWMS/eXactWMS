...
begin
  ...
  try
    ...
    if AParams.ContainsKey('datainicialpedido') then Begin
      Try
        Fconexao.Query.ParamByName('pDataInicialpedido').Value := StrToDate(AParams.Items['datainicialpedido']);
    ...
    if AParams.ContainsKey('datafinalpedido') then Begin
      Try
        Fconexao.Query.ParamByName('pDataFinalPedido').Value := StrToDate(AParams.Items['datafinalpedido']);
        vParamsOk := vParamsOk + 1;
      Except
    ...
    if AParams.ContainsKey('datainicialproducao') then Begin
       Try
         Fconexao.Query.ParamByName('pDataInicialproducao').Value := StrToDate(AParams.Items['datainicialproducao']);
         vParamsOk := vParamsOk + 1;
       Except
    ...
    if AParams.ContainsKey('datafinalproducao') then Begin
       Try
         Fconexao.Query.ParamByName('pDataFinalProducao').Value := StrToDate(AParams.Items['datafinalproducao']);
         vParamsOk := vParamsOk + 1;
       Except
    ...
    if (AParams.ContainsKey('recheckout')) and (AParams.Items['recheckout'].ToInteger()=1) then
       Fconexao.Query.ParamByName('pProcessoId').Value := 12
    Else
       Fconexao.Query.ParamByName('pProcessoId').Value := 10;
    if DebugHook <> 0 then
...
begin
  try
    ...
    If DebugHook <> 0 then
       Fconexao.Query.Sql.SaveToFile('EtiquetaPorVolumeFrac.Sql');
    Fconexao.Query.Open;
...
begin
  ...
  Try
    ...
    End;
    try
      Fconexao.Query.Sql.Add('Declare @PedidoId Integer = ' +pJsonObject.GetValue<Integer>('pedidoId').ToString());
      ...
      Fconexao.Query.Sql.Add('   From PedidoVolumes Pv'); //01062024 nao cancelar volume já cancelado
      ...
      Fconexao.Query.Sql.Add('         (@PedidoVolumeId = 0 or @PedidoVolumeId = PedidoVolumeId) and');
      Fconexao.Query.Sql.Add('          De.ProcessoId = (Select Max(ProcessoId) From DocumentoEtapas where documento = pv.uuid) And De.ProcessoId <> 15');
      //Limpar a caixa/Baget utilizado pelo Volume
      Fconexao.Query.Sql.Add('Update CaixaEmbalagem Set PedidoVOlumeId = Null Where PedidoVolumeId = @PedidoVolumeId');
    ...
    Except ON E: Exception do
      Begin
       ...
       raise Exception.Create('Tabela: PedidoVolumes - '+TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    Qry.Close;
  ...
  try
    ...
    Else
      ...
      Begin
        ...
        jsonVolume.AddPair('pedidoid', tjsonNumber.Create(Fconexao.Query.FieldByName('PedidoId').AsInteger));
        jsonVolume.AddPair('pedidovolumeid', tjsonNumber.Create(Fconexao.Query.FieldByName('PedidoVolumeId').AsInteger));
        jsonVolume.AddPair('volumetipo', Fconexao.Query.FieldByName('VolumeTipo').AsString);
        jsonVolume.AddPair('embalagem', Fconexao.Query.FieldByName('embalagem').AsString);
        jsonVolume.AddPair('descricao', Fconexao.Query.FieldByName('descricao').AsString);
        jsonVolume.AddPair('identificacao', Fconexao.Query.FieldByName('Identificacao').AsString);
        jsonVolume.AddPair('tara', tjsonNumber.Create(Fconexao.Query.FieldByName('Tara').AsInteger));
        jsonVolume.AddPair('sequencia', tjsonNumber.Create(Fconexao.Query.FieldByName('Sequencia').AsInteger));
        jsonVolume.AddPair('volumecaixa', tjsonNumber.Create(Fconexao.Query.FieldByName('VolumeCaixa').AsInteger));
        jsonVolume.AddPair('processoid', tjsonNumber.Create(Fconexao.Query.FieldByName('ProcessoId').AsInteger));
        jsonVolume.AddPair('processo', Fconexao.Query.FieldByName('Processo').AsString);
        jsonVolume.AddPair('usuarioid', tjsonNumber.Create(Fconexao.Query.FieldByName('UsuarioId').AsInteger));
        jsonVolume.AddPair('usuario', Fconexao.Query.FieldByName('Usuario').AsString);
        jsonVolume.AddPair('demanda', tjsonNumber.Create(Fconexao.Query.FieldByName('Demanda').AsInteger));
        jsonVolume.AddPair('qtdsuprida', tjsonNumber.Create(Fconexao.Query.FieldByName('QtdSuprida').AsInteger));
        jsonVolume.AddPair('carregamentoid', tjsonNumber.Create(Fconexao.Query.FieldByName('CarregamentoId').AsInteger));
        ...
        JsonPedido.AddPair('documentodata', FormatdateTime('YYYY-MM-DD', Fconexao.Query.FieldByName('DocumentoData').AsDateTime));
        JsonDestino.AddPair('pessoaid', tjsonNumber.Create(Fconexao.Query.FieldByName('PessoaId').AsInteger));
        JsonDestino.AddPair('razao', Fconexao.Query.FieldByName('Razao').AsString);
        ...
        JsonRota.AddPair('ordem', tjsonNumber.Create(Fconexao.Query.FieldByName('Ordem').AsInteger));
        jsonVolume.AddPair('pedido', JsonPedido);
        jsonVolume.AddPair('destino', JsonDestino);
        jsonVolume.AddPair('rota', JsonRota);
        jsonVolume.AddPair('ruaid', tjsonNumber.Create(Fconexao.Query.FieldByName('RuaId').AsInteger));
        ...
        jsonVolume.AddPair('predioinicial', Fconexao.Query.FieldByName('PredioInicial').AsString);
...
begin
   try
    Fconexao.Query.Sql.Add(TuEvolutConst.GetVolumeConsultaLotes);
    ...
    Fconexao.Query.ParamByName('pOrdem').Value := 0;
    if DebugHook <> 0 then
       Fconexao.Query.Sql.SaveToFile('GetVolumeConsultaLotes.Sql');
    Fconexao.Query.Open;
    if Fconexao.Query.Isempty then Begin
      Result := TJsonArray.Create;
      Result.AddElement(TJsonObject.Create.AddPair('MSG', TuEvolutConst.QrySemDados))
...
begin
  try
    vParamCount := 0;
    Fconexao.Query.Sql.Add(TuEvolutConst.SqlGetVolumeExcel);
    If AParams.ContainsKey('datainicial') then Begin
       Fconexao.Query.ParamByName('pDataInicial').Value := FormatdateTime('YYYY-MM-DD', StrToDate(AParams.Items['datainicial']));
       Inc(vParamCount);
    End
    ...
    Else Begin
      if DebugHook <> 0 then
         Fconexao.Query.Sql.SaveToFile('VolumeExcel.Sql');
      Fconexao.Query.Open;
      if Fconexao.Query.Isempty then Begin
        Result := TJsonArray.Create;
        Result.AddElement(TJsonObject.Create.AddPair('MSG', TuEvolutConst.QrySemDados))
...
begin
  Try
    ...
    Fconexao.Query.Open;
    If DebugHook <> 0 then
       Fconexao.Query.Sql.SaveToFile('PedidoVOlumeLotes.Sql');
    if Fconexao.Query.Isempty then
    Begin
      Result := TJsonArray.Create;
      Result.AddElement(TJsonObject.Create(TJSONPair.Create('Erro', TuEvolutConst.QrySemDados)));
...
begin
  ...
  try
    ...
    Fconexao.Query.ParamByName('pPedidoVolumeId').Value := pPedidoVolumeId;
    if DebugHook <> 0 then
       Fconexao.Query.Sql.SaveToFile('IdentificaVolumecxaFechada.Sql');
    Fconexao.Query.Open;
    while Not Fconexao.Query.Eof do
...
begin
  ...
  Except ON E: Exception do
    Begin
      raise Exception.Create('Tabela: PedidoVolumeHistorico - ' + TUtil.TratarExcessao(E.Message));
    End;
  end;
  Fconexao.Query.Close;
end;
