...
type
  ...
  public
    ...
    Function GetVolumeComDivergencia(pPedidoVolumeId, pEnderecoId: Integer): TJsonArray;
...
begin
  ...
  Try
    ...
    Try
      Query.Close;
      Query.Sql.Clear;
      Query.Sql.Add(TuEvolutConst.SqlPedidoVolumeLacresCarga);
      Query.ParamByName('pCargaId').Value := pCargaId;
      If DebugHook <> 0 then
         Query.SQL.SaveToFile('VolumeLacresCarga.Sql');
      Query.Open;
    ...
    Except On E: Exception do Begin
      Raise Exception.Create('Processo: GetVolumeLacresCarga - '+TUtil.TratarExcessao(E.Message));
      End;
    End;
  Finally
    FreeAndNil(Query);
...
begin
  ...
  Try
    ...
    Try
      ...
      Query.Close;
      Query.Sql.Clear;
      Query.Sql.Add('Delete PedidoVolumeLacres Where PedidoVolumeId = '+pJsonObjectLacres.GetValue<String>('volumeid'));
      Query.ExecSQL;
      For xLacre := 0 To Pred(JsonArrayLacres.Count) do Begin
        Query.Close;
        Query.Sql.Clear;
        Query.Sql.Add('Insert Into PedidoVolumeLacres (PedidoVolumeId, lacrenr, status) Values (');
        Query.Sql.Add('     '+pJsonObjectLacres.GetValue<String>('volumeid')+', ');
        Query.Sql.Add('     '+JsonArrayLacres.Items[xLacre].GetValue<string>('lacre')+', 1)');
        Query.ExecSQL;
      End;
    ...
    Except On E: Exception do Begin
      ...
      Raise Exception.Create('Processo: VolumesLacres - '+TUtil.TratarExcessao(E.Message));
      End;
    End;
  Finally
    FreeAndNil(Query);
...
Begin
  Try
    ...
    Except ON E: Exception do
      Begin
        Raise Exception.Create('Processo: PedidoVolume/AtualizarKardex - '+TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    pQuery.Close;
...
function TServicePedidoVolume.CaixaSeparacao(pPedidoVolumeId, pCaixaEmbalagemId : Integer): TJsonArray;
var vQry: TFdQuery;
begin
  Try
    vQry := TFdQuery.Create(Nil);
    try
      ...
      vQry.ExecSQL;
      Result.AddElement(TJsonObject.Create.AddPair('Ok', 'Caixa Alterada com sucesso!'));
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: CaixaSeparacao - '+TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
   vQry.Close;
...
begin
  Try
    vQry            := TFdQuery.Create(nil);
    vQryCheckPedido := TFdQuery.Create(nil);
    ...
    try
      ...
      vQry.Sql.Add('if object_id ('#39+'tempdb..#VolumeNaoExpedido'+#39+') is not null drop table #VolumeNaoExpedido');
      vQry.Sql.Add('if object_id ('#39+'tempdb..#VolumeExpedido'+#39+') is not null drop table #VolumeExpedido');
      vQry.SQL.Add('if object_id ('#39+'tempdb..#VolumeProcesso'+#39+') is not null drop table #VolumeProcesso');
...
begin
  ...
  Try
    ...
    try
      Result := TJsonArray.Create;
      Query.SQL.Add('Declare @Documento VarChar(36) = (Select Uuid from PedidoVolumes Where PedidoVolumeId = '+pPedidoVolumeId.ToString+')');
      Query.SQL.Add('Delete DocumentoEtapas');
      Query.SQL.Add('Where DocumentoEtapaid = (Select Max(DocumentoEtapaId) from DocumentoEtapas where Documento = @Documento)');
      Query.ExecSql;
      Result := TJsonArray.Create;
...
begin
  ...
  Try
    ...
    try
      ...
      for xArray := 0 to Pred(pJsonObject.GetValue<TJsonArray>('pedido').Count) do Begin
  ...
  Try
    ...
    try
      ...
      if pJsonObject.GetValue<Integer>('operacao') = 1 then Begin
         if pJsonObject.GetValue<Integer>('consolidado', 0) = 1 then
...
begin
  ...
  Try
    ...
    Try
      ...
      if AParams.ContainsKey('usuarioid') then
         pUsuarioId := AParams.Items['usuarioid'].ToInteger;
      Query.Sql.Add(TuEvolutConst.SqlAuditoriaVolumes);
      Query.ParamByName('pPedidoid').Value := pPedidoId;
      Query.ParamByName('pDocumentoNr').Value := pDocumentoNr;
      if pDataInicial <> 0 then
         Query.ParamByName('pDtInicio').Value := FormatDatetime('YYYY-MM-DD', pDataInicial)
      Else
         Query.ParamByName('pDtInicio').Value := 0;
      if pDataFinal <> 0 then
         Query.ParamByName('pDtFinal').Value := FormatDatetime('YYYY-MM-DD', pDataFinal)
      Else
         Query.ParamByName('pDtFinal').Value := 0;
      ...
      Query.ParamByName('pLinhaId').Value := pLinhaId;
      Query.ParamByName('pZonaId').Value := pZonaId;
      Query.ParamByName('pTipoVolume').Value := pTipoVolume;
      Query.ParamByName('pUsuarioId').Value := pUsuarioId;
      If DebugHook <> 0 Then
         Query.Sql.SaveToFile('AuditoriaVolumes.Sql');
      Query.Open;
      if Query.IsEmpty then Begin
         Result := TJsonArray.Create;
         Result.AddElement(TJsonObject.Create.AddPair('Erro', 'Sem Dados para a consulta.'))
...
begin
  ...
  Try
    Query.Connection := Connection;
    try
      Query.Sql.Add(TuEvolutConst.SqlGetdshvolumeevolucao_quantidade);
      Query.ParamByName('pData').Value := pDataIni;
      Query.ParamByName('pRotaId').Value := pRotaId;
      Query.ParamByName('pZonaId').Value := pZonaId;
      Query.ParamByName('pCodPessoaERP').Value := pCodPessoaERP;
      if DebugHook <> 0 then
         Query.Sql.SaveToFile('Getdshvolumeevolucao_quantidade.Sql');
      Query.Open;
      if Query.IsEmpty then Begin
        Result := TJsonArray.Create;
        Result.AddElement(TJsonObject.Create(tJsonPair.Create('Erro', TuEvolutConst.QrySemDados)))
...
begin
  ...
  Try
    Query.Connection := Connection;
    try
      Query.Sql.Add(TuEvolutConst.SqlGetdshvolumeevolucao_Unidades);
      Query.ParamByName('pData').Value := pDataIni;
      Query.ParamByName('pRotaId').Value := pRotaId;
      Query.ParamByName('pZonaId').Value := pZonaId;
      Query.ParamByName('pCodPessoaERP').Value := pCodPessoaERP;
      if DebugHook <> 0 then
         Query.Sql.SaveToFile('Getdshvolumeevolucao_Unidades.Sql');
      Query.Open;
      if Query.IsEmpty then Begin
         Result := TJsonArray.Create;
         Result.AddElement(TJsonObject.Create(tJsonPair.Create('Erro', TuEvolutConst.QrySemDados)))
...
begin
  ...
  Try
    vQry := TFdQuery.Create(Nil);
    vQrySeparacao := TFdQuery.Create(Nil);
    try
      vQry.Connection := Connection;
      vConexao := vQry.Connection.Name;
      vQrySeparacao.connection := Connection;
...
//         vQry.connection.Commit;
  ...
  Finally
    If Assigned(vQry) then FreeAndNil(vQry);
    If Assigned(vQrySeparacao) then FreeAndNil(vQrySeparacao);
  End;
end;
...
begin
  ...
  Try
    ...
    try
      Query.Close;
      Query.Sql.Clear;
      Query.Sql.Add(TuEvolutConst.SqlPedidoCxaFechadaCheckOut);
      Query.ParamByName('pPedidoVolumeId').Value := pPedidoVolumeId;
      if DebugHook <> 0 then
         Query.Sql.SaveToFile('GetPedidoCxaFechadaCheckOut.Sql');
      Query.Open;
      ...
      Begin
        ...
        case Query.FieldByName('ProcessoId').AsInteger of
          2: vErro  := 'Imprima a Etiqueta.';
          10, 11, 12, 13: vErro := 'Processo Volume: ' + Query.FieldByName('Processo').AsString;
          15: vErro := 'Volume Cancelado.';
        end;
        If Query.FieldByName('BloqueioInventario').asInteger = 1 then Begin
        ...
        Else
        Begin
          Result.AddPair('volume', Query.ToJsonArray);
          Query.Close;
          Query.Sql.Clear;
          Query.Sql.Add(TuEvolutConst.SqlPedidoCxaFechadaCheckOutProd);
          Query.ParamByName('pPedidoVolumeId').Value := pPedidoVolumeId;
          Query.Open();
          if Query.IsEmpty then Begin
          ...
          End
          Else Begin
            Result.AddPair('produto', Query.ToJsonArray);
            Query.Close;
            Query.Sql.Clear;
            Query.Sql.Add(TuEvolutConst.SqlPedidoCxaFechadaCheckOutCodBarras);
            Query.ParamByName('pPedidoVolumeId').Value := pPedidoVolumeId;
            Query.Open();
            Result.AddPair('codbarras', Query.ToJsonArray);
          End;
          //Registrar Inicio do Processo de CheckOut
          Query.Close;
          if vProcessoId <> 9 then Begin
             Query.Sql.Clear;
             Query.Sql.Add('declare @uuid UNIQUEIDENTIFIER = (Select uuid From PedidoVolumes ');
             Query.Sql.Add('                                  where ' + 'PedidoVolumeId = '+pPedidoVolumeId.ToString() + ')');
             Query.Sql.Add(TuEvolutConst.SqlRegistrarDocumentoEtapa);
             Query.ParamByName('pProcessoId').Value := 9;
             Query.ParamByName('pUsuarioId').Value  := pusuarioid;
...
begin
  ...
  Try
    ...
    Except ON E: Exception do
      Begin;
        raise Exception.Create('Processo: GetPedidoVolumeProdutoLote - ' + TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    FreeAndNil(vQry);
...
function TServicePedidoVolume.GetVolumeComDivergencia(pPedidoVolumeId, pEnderecoId : Integer) : TJsonArray;
var vQry: TFdQuery;
begin
  Try
    vQry := TFdQuery.Create(Nil);
    Try
      ...
      vQry.ParamByName('pPedidoVolumeId').Value := pPedidoVolumeId;
      vQry.ParamByName('pEnderecoId').Value     := pEnderecoId;
      if DebugHook <> 0 then
...
begin
  Try
    ...
    Except On E: Exception do
      Begin
        raise Exception.Create('Processo: Volumes/RegistrarDocumentoEtapa - '+TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    FreeAndNil(vQry);
...
begin
  try
    ...
    try
      ...
      vQry.Sql.Add(TuEvolutConst.SqlRegistrarDocumentoEtapa);
      vQry.Sql.Add('--  ');
      vQry.Sql.Add('Update PedidoVolumes Set Expedido = 1 Where PedidoVolumeId = '+pJsonDocumentoEtapa.GetValue<Integer>('pedidovolumeid').ToString());
      vQry.ParamByName('pProcessoId').Value := pJsonDocumentoEtapa.GetValue<Integer>('processoid');
      vQry.ParamByName('pUsuarioId').Value := pJsonDocumentoEtapa.GetValue<Integer>('usuarioid');
      vQry.ParamByName('pTerminal').Value := pJsonDocumentoEtapa.GetValue<String>('estacao');
...
begin
  Try
    ...
    Except ON E: Exception do
      Begin
        vQry.connection.Rollback;
        raise Exception.Create('Processo: RegistrarDocumentoEtapaSemBaixaEstoque - '+TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    FreeAndNil(vQry);
...
function TServicePedidoVolume.ResetSeparacao(pPedidoVolumeId: Integer) : TJsonArray;
var vQry: TFdQuery;
begin
  Result := TJsonArray.Create;
  Try
    ...
    try
      ...
      vQry.ExecSQL;
      Result.Add(TJsonObject.Create.AddPair('Ok', 'Operação concluída com sucesso!'));
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: ResetSeparaçãoVolume - ' + TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    FreeAndNil(vQry);
...
Var vQry, vQryAtualiza: TFdQuery;
    ...
    VolCxaFechadaExpedicao, vVolCxaFracionadaExpedicao, vExpedicaoOffLine : Boolean;
begin
//  If (pJsonColetaRegistro = Nil) then
//     Tutil.Gravalog('001 - Ocorreu erro em Services.PedidoVolume.SalvarColetaComRegistro - Dados Incorretos na requisição. ');
//     Raise Exception.Create('Dados incorretos na requisição');
  Try
    VolCxaFechadaExpedicao     := False;
    vVolCxaFracionadaExpedicao := False;
    vExpedicaoOffLine          := False;
    ...
    Try
      ...
      Try
        ...
        End;
      Except ON E: Exception do Begin
        Tutil.Gravalog('XXX - Ocorreu erro em Services.PedidoVolume.SalvarColetaComRegistro - Dados Incorretos na requisição. ');
        End;
      End;
      vQry.Close;
      vQry.SQL.Clear;
      ...
      For xColeta := 0 to Pred(pJsonArrayColeta.Count) do Begin
        ...
        While (Not vQry.Eof) do Begin // and (vQtdConferida>0)
          ...
          Try
            vQryAtualiza.ExecSQL;
          Except On E: Exception do Begin
            Tutil.Gravalog('002 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro - . Atualizando Dados Separacao/Volumelote. '+E.Message);
            raise Exception.Create('Processo: SalvarColetaComRegistro: Atualizando Dados Separacao/Volumelote');
            End;
          End;
          vQry.Next
        End;
      ...
      Try
        vQry.ExecSQL;
      Except On E: Exception do Begin
        Tutil.Gravalog('003 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro - . Atualizando Dados PedidoVOlumeSeparação. '+E.Message);
        raise Exception.Create('Processo: SalvarColetaComRegistro: Atualizando PedidoVolumeSeparacao');
        End;
      End;
      if pJsonColetaRegistro.GetValue<Integer>('operacao') = 1 then Begin
        // Status do Volume
        ...
        Try
          vQry.ExecSQL;
        Except ON E: Exception do Begin
          Tutil.Gravalog('004 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro - . Atualizando Volume. '+E.Message);
          raise Exception.Create('Processo: SalvarColetaComRegistro: Atualizando Volume');
          End;
        End;
      ...
      //Registrar Fracionado na Expedição - Verificar se está no local correto
        if (pJsonColetaRegistro.GetValue<Integer>('operacao') = 1) and (vVolCxaFracionadaExpedicao and vExpedicaoOffLine) then Begin
           ...
           if pJsonColetaRegistro.GetValue<Integer>('usuarioid') > 0 then Begin
              Try
                vQry.ExecSQL;
              Except ON E: Exception do Begin
                Tutil.Gravalog('005 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro - . Atualizando StatusVolume - Expedição. '+E.Message);
                raise Exception.Create('Processo: SalvarColetaComRegistro: Atualizando StatusVolume - Expedição');
                End;
              End;
           End;
        End;
        //Atualizar a Caixa/Baget usada no volume
        //If pJsonColetaRegistro.GetValue<Integer>('pedidovolumeid') > 0 then Begin
           vQry.Close;
           vQry.Sql.Clear;
           ...
           Try
             vQry.ExecSQL;
           Except On E: Exception do Begin
             Tutil.Gravalog('006 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro - . Atualizando Caixa Badger. '+E.Message);
             raise Exception.Create('Processo: SalvarColetaComRegistro: Atualizando Caixa/Badget');
             End;
           End;
        //End;
        vQry.Close;
        ...
        Try
          vQry.ExecSQL;
        Except On E: Exception do Begin
          Tutil.Gravalog('007 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro - . Atualizando StatusPedido. '+E.Message);
          raise Exception.Create('Processo: SalvarColetaComRegistro: Atualizando StatusPedido');
          End;
        End;
    ...
    Except ON E: Exception do
      Begin
        Tutil.Gravalog('008 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro - . Cancelando transação. '+E.Message);
        vQry.connection.Rollback;
  ...
  Finally
    Try
      If Assigned(vQry) then
         FreeAndNil(vQry);
      If Assigned(vQryAtualiza) then
         FreeAndNil(vQryAtualiza);
    Except ON E: Exception do Begin
      Tutil.Gravalog('009 - Ocorreu ERRO em Services.PedidoVolume.SalvarColetaComRegistro. Finalizado(Querys). '+E.Message);
      End;
    End;
  End;
end;

function TServicePedidoVolume.SaveApanheProdutos(pJsonArray: TJsonArray) : TJsonArray;
Var
...
begin
  Try
    ...
    Try
      ...
      vQryAtualiza.Sql.Add('                           (:poperpedidovolumeid, :poperloteid, :poperenderecoid, :poperquantidade,	:poperqtdsuprida,	');
      vQryAtualiza.Sql.Add('                           	(Select Processoid from ProcessoEtapas where Descricao = :poperprocessoetapa), :poperdata,	:poperhora,	:poperusuarioid,	:poperestacao)');
      For xColeta := 0 to Pred(pJsonArray.Count) do Begin
        vQry.Close();
        vQry.ParamByName('pCodProduto').Value := pJsonArray.Items[xColeta].GetValue<Integer>('codproduto');
        vQry.ParamByName('pPedidoVolumeId').Value := pJsonArray.Items[xColeta].GetValue<Integer>('pedidovolumeid');
        vQry.Open;
        vQtdConferida := pJsonArray.Items[xColeta].GetValue<Integer>('qtdsuprida');
        // pJsonArray.Items[xProd].GetValue<Integer>('quantidade');  //Quantidade Suprida
        While (Not vQry.Eof) do Begin // and (vQtdConferida>0)
          vQryAtualiza.Close;
          ...
          vQryAtualiza.ParamByName('pPedidoVolumeId').Value := pJsonArray.Items[xColeta].GetValue<Integer>('pedidovolumeid');
          vQryAtualiza.ParamByName('pLoteid').Value := pJsonArray.Items[xColeta].GetValue<Integer>('loteid');
          // vQry.FieldByName('LoteId').AsInteger;
          vQryAtualiza.ParamByName('pUsuarioId').Value := pJsonArray.Items[xColeta].GetValue<Integer>('usuarioid');
          vQryAtualiza.ParamByName('pDtSeparacao').Value := FormatDatetime('YYYY-MM-DD', StrToDate(pJsonArray.Items[xColeta].GetValue<String>('data')));
          // pJsonArray.Items[xProd].GetValue<String>('data'); //
          vQryAtualiza.ParamByName('pHrSeparacao').Value := pJsonArray.Items[xColeta].GetValue<String>('horariotermino');
          vQryAtualiza.ParamByName('pTerminal').Value := pJsonArray.Items[xColeta].GetValue<String>('terminal');

          // vQryAtualiza.ParamByName('pQtdeRes').Value        := pJsonArray.Items[xColeta].GetValue<Integer>('demanda'); //vQtdConferida;
          vQryAtualiza.ParamByName('pQtdSuprida').Value := pJsonArray.Items[xColeta].GetValue<Integer>('qtdsuprida');
          // vQtdConferida;
          vQryAtualiza.ParamByName('poperquantidade').Value := pJsonArray.Items[xColeta].GetValue<Integer>('qtdsuprida');
          // vQtdConferida;
          vQryAtualiza.ParamByName('poperqtdsuprida').Value := pJsonArray.Items[xColeta].GetValue<Integer>('qtdsuprida');
          // vQtdConferida;

          vQtdConferida := 0;
          // End;
          // Gravar acompanhamento da operacao para an�lise de resultados
          vQryAtualiza.ParamByName('poperPedidoVolumeId').Value := pJsonArray.Items[xColeta].GetValue<Integer>('pedidovolumeid');
          vQryAtualiza.ParamByName('poperLoteid').Value := pJsonArray.Items[xColeta].GetValue<Integer>('loteid');
          // vQry.FieldByName('LoteId').AsInteger;
          vQryAtualiza.ParamByName('poperEnderecoid').Value := pJsonArray.Items[xColeta].GetValue<Integer>('enderecoid');
          // vQry.FieldByName('EnderecoId').AsInteger;
          vQryAtualiza.ParamByName('poperquantidade').Value := pJsonArray.Items[xColeta].GetValue<Integer>('demanda');
          // vQry.FieldByName('Quantidade').AsInteger;
          vQryAtualiza.ParamByName('poperqtdsuprida').Value := pJsonArray.Items[xColeta].GetValue<Integer>('qtdsuprida');
          // pJsonArray.Items[xProd].GetValue<Integer>('quantidade');
          vQryAtualiza.ParamByName('poperUsuarioId').Value := pJsonArray.Items[xColeta].GetValue<Integer>('usuarioid');
          vQryAtualiza.ParamByName('pOperData').Value := FormatDatetime('YYYY-MM-DD', StrToDate(pJsonArray.Items[xColeta].GetValue<String>('data')));
          // pJsonArray.Items[xProd].GetValue<String>('data'); //
          vQryAtualiza.ParamByName('pOperHora').Value := pJsonArray.Items[xColeta].GetValue<String>('horariotermino');
          vQryAtualiza.ParamByName('poperestacao').Value := pJsonArray.Items[xColeta].GetValue<String>('terminal');
          vQryAtualiza.ParamByName('poperprocessoetapa').Value := pJsonArray.Items[xColeta].GetValue<String>('processoetapa');
...
begin
  ...
  Try
    Query.Connection := Connection;
    try
      Query.SQL.Add(TuEvolutConst.SqlVolumemExpedidoDia);
      //FConexao.Query.ParamByName('pData').Value      := FormatDateTime('YYYY-MM-DD',  pData);
      Query.ParamByName('pUsuarioId').Value := pUsuarioId;
      Query.SQL.SaveToFile('VolumeExpedidoDia.Sql');
      Query.Open;
      if Query.IsEmpty() then Begin
         Result := TJsonArray.Create;
         Result.AddElement(TJsonObject.Create.AddPair('Erro', TuEvolutConst.QrySemDados))
...
function TServicePedidoVolume.VolumeLoteSubstituicao(pJsonObjectLotes : TJsonObject): TJsonArray;
var vQryLoteSubstituicao: TFdQuery;
    xLotes: Integer;
    JsonArrayLotes: TJsonArray;
begin
  Try
    vQryLoteSubstituicao := TFdQuery.Create(Nil);
    try
      vQryLoteSubstituicao.Connection := Connection;
  ...
  Finally
    FreeAndNil(vQryLoteSubstituicao);
  End;
end;
...
begin
  ...
  Try
    ...
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: VolumeRegistrarExpedicao - ' + Tutil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    FreeAndNil(vQry);
