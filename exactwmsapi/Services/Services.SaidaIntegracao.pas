...
Const SqlSaidaInsert =
      'Begin Try' + sLineBreak +
      '  If Not Exists (Select PedidoId From Pedido '+sLineBreak+
      '                 Where PessoaId = @PessoaId '+sLineBreak+
      '                   and DocumentoNr = @DocumentoNr and RegistroERP = @RegistroERP) Begin'+sLineBreak +
      '     Insert Into Pedido (OperacaoTipoId, operacaonaturezaid, PessoaId, DocumentoNr, '+sLineBreak+
      '                         DocumentoOriginal, DocumentoData, DtInclusao, HrInclusao,	'+sLineBreak +
      '                         ArmazemId, RegistroERP, Status, uuid) Values (@OperacaoTipoId, '+sLineBreak+
      '                         (select operacaonaturezaid From OperacaoNatureza where Descricao = @OperacaoNatureza), '+sLineBreak +
      ...
      '     Insert Into DocumentoEtapas Values ((select uuid from pedido where PessoaId = @PessoaId and DocumentoNr = @DocumentoNr and RegistroERP = @RegistroERP), '+sLineBreak+
      ...
      '     Delete from PedidoProdutos '+sLineBreak+
      '     Where PedidoId = (Select Pedidoid From Pedido Where PessoaId = @PessoaId and DocumentoNr = @DocumentoNr and OperacaoTipoid = @OperacaoTipoId and RegistroERP = @RegistroERP)'+sLineBreak +
      '     Update Pedido' + sLineBreak +
      '        Set OperacaoTipoId = @OperacaoTipoId' + sLineBreak +
      '          , DocumentoData  = @DocumentoData' + sLineBreak +
      '          , ArmazemId      = 1' + sLineBreak +
      '          , RegistroERP    = @RegistroERP' + sLineBreak +
      '     Where PessoaId = @PessoaId and DocumentoNr = @DocumentoNr and OperacaoTipoid = @OperacaoTipoId'+sLineBreak+
      '       and RegistroERP = @RegistroERP'+sLineBreak +
      '  End' + sLineBreak + sLineBreak +
      '  Select PedidoId From Pedido '+sLineBreak+
      '  Where PessoaId = @PessoaId and DocumentoNr = @DocumentoNr and OperacaoTipoid = @OperacaoTipoId'+sLineBreak +
      'End'+sLineBreak+
      'Try Begin Catch' + sLineBreak +
...
begin
  ...
  Try
    ...
    Try
      ...
      Query.Sql.Add(TuEvolutConst.jsonSaidaRetornoCheckInFinalizadoV2);
      Query.ParamByName('pVersao').Value := pVersao;
      if DebugHook <> 0 then
         Query.Sql.SaveToFile('ConsutaSaidaIntegracao_Service_' + pVersao + '.Sql');
      Query.Open();
      if (Query.IsEmpty) or (Query.FieldByName('ConsultaRetorno').AsString = '') then Begin
          Result.AddElement(TJsonObject.Create.AddPair('status', IfThen(pVersao = 'V2', '200', '200')).AddPair('pedidoid',
                            TJsonNumber.Create(0)).AddPair('documentoerp', '').AddPair('mensagem', TuEvolutConst.QrySemDados));
      End
      Else Begin
...
begin // Processo lento
  ...
  vQryDestinatario     := TFdQuery.Create(Nil);
  vQryPedStatus        := TFdQuery.Create(Nil);
  vQryProduto          := TFdQuery.Create(Nil);
  vQryProdutoCodBarras := TFdQuery.Create(Nil);
  vQryItens            := TFdQuery.Create(Nil);
  Try
    vQryInsert.Connection           := Connection;
    vQryDestinatario.Connection     := Connection;
    vQryPedStatus.Connection        := Connection;
    vQryProduto.Connection          := Connection;
    vQryProdutoCodBarras.Connection := Connection;
    ...
    vQryInsert.Sql.Add('Insert Into JsonIntegracao Values (NewId(), ' + #39 + ArraySaidas.toString() + #39 + ')');
    If DebugHook <> 0 Then
    ...
    for xSaida := 0 to Pred(ArraySaidas.Count) do Begin
      Try
        ...
        Begin
          ...
          vDocumentoOriginal := ArraySaidas.Items[xSaida].getValue<String>('documentooriginal');
        End;
        // Salvar Destinatário
        jsonDestinatario := ArraySaidas.Items[xSaida].getValue<TJsonObject>('destinatario');
        vQryDestinatario.Sql.Add(TuEvolutConst.SqlSalvarDestinatario);
        vQryDestinatario.ParamByName('pPessoaTipoId').Value := 1;
        vQryDestinatario.ParamByName('pCodPessoaERP').Value := jsonDestinatario.getValue<Integer>('destinatarioid');
        vQryDestinatario.ParamByName('pRazao').Value        := jsonDestinatario.getValue<String>('razao');
        vQryDestinatario.ParamByName('pFantasia').Value     := jsonDestinatario.getValue<String>('fantasia');
        vCnpj := jsonDestinatario.getValue<String>('cnpj');
        vQryDestinatario.ParamByName('pCnpjCpf').Value   := StringReplace(StringReplace(StringReplace(vCnpj, '.', '', [rfReplaceAll]), '-', '', [rfReplaceAll]), '/', '', [rfReplaceAll]);
        vQryDestinatario.ParamByName('pEmail').Value     := jsonDestinatario.getValue<String>('email');
        ...
        vQryPedStatus.Sql.Add('Declare @PessoaId Integer = (Select PessoaId From Pessoa Where CodPessoaERP = '+ jsonDestinatario.getValue<Integer>('destinatarioid').toString()+' and PessoaTipoId = 1)');
        ...
        vQryPedStatus.Open;
        if (Not vQryPedStatus.IsEmpty) and (vQryPedStatus.FieldByName('ProcessoId').AsInteger > 1) then Begin
           vQryPedStatus.Close;
           raise Exception.Create('Não é permitido o reenvio deste ressuprimento: ' + ArraySaidas.Get(xSaida).getValue<String>('registroerp') + '.');
           JsonRetorno := TJsonObject.Create;
           if xSaida = 0 then
              JsonRetorno.AddPair('Retorno', 'Erro: Não é permitido o reenvio deste ressuprimento.');
           JsonRetorno.AddPair('registroerp', ArraySaidas.Get(xSaida).getValue<String>('registroerp'));
           Result.AddElement(JsonRetorno);
           vQryPedStatus.Close;
        End
        Else Begin;
          vQryPedStatus.Close;
          vQryInsert.Sql.Add('Declare @PedidoId Integer = ' + ArraySaidas.Get(xSaida).getValue<Integer>('saidaid', 0).toString());
          vQryInsert.Sql.Add('Declare @OperacaoTipoId Integer = (Select OperacaoTipoId From OperacaoTipo Where Upper(Descricao) = ' + QuotedStr(UpperCase('Saída')) + ')');
          vQryInsert.Sql.Add('Declare @PessoaId Integer = (Select PessoaId From Pessoa Where CodPessoaERP = ' + jsonDestinatario.getValue<Integer>('destinatarioid').toString() + ' and PessoaTipoId = 1)');
          vQryInsert.Sql.Add('Declare @DocumentoNr VarChar(20) = ' + QuotedStr(ArraySaidas.Get(xSaida).getValue<String>('documentonr')));
          vQryInsert.Sql.Add('Declare @DocumentoData Int = (Select IdData From Rhema_Data Where Data =' + #39 + ArraySaidas.Get(xSaida).getValue<String>('documentodata') + #39 + ')');
          vQryInsert.Sql.Add('Declare @RegistroERP Varchar(36) = ' + QuotedStr(ArraySaidas.Get(xSaida).getValue<String>('registroerp')));
          vQryInsert.Sql.Add('Declare @OperacaoNatureza VarChar(30) = ' + QuotedStr(vOperacaoNatureza));
          vQryInsert.Sql.Add('Declare @DocumentoOriginal Varchar(20)  = ' + QuotedStr(vDocumentoOriginal));
          ...
          vQryInsert.Close;
          if vSaidaId > 0 then Begin
             jsonItens := ArraySaidas.Get(xSaida).getValue<TJsonArray>('itens');
             For xItens := 0 To Pred(jsonItens.Count) do Begin
               vQryProduto.Close;
               vQryProduto.Sql.Clear;
               vQryProduto.Sql.Add(TuEvolutConst.SqlInsProduto);
               vQryProduto.ParamByName('pCodProduto').Value := jsonItens.Items[xItens].getValue<Integer>('produtoid', 0);
               vQryProduto.ParamByName('pDescricao').Value := jsonItens.Items[xItens].getValue<String>('descricao');
               ...
               vQryProduto.ParamByName('pFatorConversao').Value := jsonItens.Items[xItens].getValue<Integer>('embalagempadrao');
               ...
               vQryProduto.ParamByName('pEan').Value := jsonItens.Get(xItens).getValue<String>('ean');
               vQryProduto.ExecSQL;
               vQryProduto.Close;
               ...
               vQryProdutoCodBarras.ParamByName('pCodProdutoERP').Value := jsonItens.Get(xItens).getValue<Integer>('produtoid', 0);
               vQryProdutoCodBarras.ParamByName('pCodBarras').Value := jsonItens.Get(xItens).getValue<String>('ean');
               vQryProdutoCodBarras.Sql.Add('--pCodProdutoERP = ' + jsonItens.Get(xItens).getValue<Integer>('produtoid', 0).toString);
               vQryProdutoCodBarras.Sql.Add('--pCodBarras = ' + jsonItens.Get(xItens).getValue<String>('ean'));
               If DebugHook <> 0 Then
               ...
               vQryItens.ParamByName('pQuantidade').Value := JsonItens.Items[xItens].getValue<Integer>('quantidade', 0);
               vQryItens.ParamByName('pEmbalagemPadrao').Value := vEmbalagemPadrao;
               vQryItens.ParamByName('pCodProduto').Value := jsonItens.Get(xItens).getValue<Integer>('produtoid', 0);
               vQryItens.ParamByName('pItemId').Value     := StrToIntDef(GetValueInJson((jsonItens.Items[xItens] as TJsonObject), 'itemid'), 0);
               vQryItens.ExecSQL;
             End;
          End;
  ...
  Finally
    ...
    FreeAndNil(vQryDestinatario);
    FreeAndNil(vQryPedStatus);
    FreeAndNil(vQryProduto);
    FreeAndNil(vQryProdutoCodBarras);
...
Var
  vQryRetorno, vQryRetornoVolumes, vQryRetornoLotes, vQryRegistrarRetorno, vQryLacres : TFdQuery;
...
begin
  Result := TJsonArray.Create;
  vQryRetorno          := TFdQuery.Create(Nil);
  vQryRetornoVolumes   := TFdQuery.Create(Nil);
  vQryRetornoLotes     := TFdQuery.Create(Nil);
  vQryRegistrarRetorno := TFdQuery.Create(Nil);
  vQryLacres           := TFdQuery.Create(Nil);
  Try
    vQryRetorno.Connection          := Connection;
    vQryRetornoVolumes.Connection   := Connection;
    vQryRetornoLotes.Connection     := Connection;
    vQryRegistrarRetorno.Connection := Connection;
    vQryLacres.Connection           := Connection;
    Try
      ...
      End
      Else Begin
          While Not vQryRetorno.Eof do Begin
            ...
            vQryRetornoVolumes.Sql.Add(TuEvolutConst.SqlSaidaIntegracaoRetornoVolume);
            ...
            While Not vQryRetornoVolumes.Eof do begin
              ...
              jsonPedidoVolume.AddPair('identificacaocaixa', TJsonNumber.Create(vQryRetornoVolumes.FieldByName('identificacaoCaixa').AsInteger));
              vQryLacres.Close;
              vQryLacres.SQL.Clear;
              vQryLacres.SQL.Add('select pedidovolumeid, lacrenr from PedidoVolumeLacres Where PedidoVolumeId = '+vQryRetornoVolumes.FieldByName('PedidoVolumeId').AsString);
              vQryLacres.Open;
              If vQryLacres.IsEmpty then
                 jsonPedidoVolume.AddPair('lacres', '[]')
              Else
                 jsonPedidoVolume.AddPair('lacres', vQryLacres.ToJSONArray());;
            ...
            if (pVersao = 'V2') or ((pVersao = 'V3') and (vQryRetorno.FieldByName('ProcessoId').AsInteger = 15) or (vQryRetorno.FieldByName('ProcessoId').AsInteger = 31)) then Begin
              ...
              //if (vQryRetorno.FieldByName('ProcessoId').AsInteger = 15) or (vQryRetorno.FieldByName('ProcessoId').AsInteger = 31) then
    ...
    Except On E: Exception do Begin
      ...
      Result.AddElement(TJsonObject.Create.AddPair('status', '500')
                                   .AddPair('pedidoid', pPedido).AddPair('documentoerp', pPedido)
                                   .AddPair('mensagem', 'Integração Saída: Retorno - ' + TUtil.TratarExcessao(E.Message) ));
      End;
    End;
  Finally
    FreeAndNil(vQryRetorno);
    FreeAndNil(vQryRetornoVolumes);
    FreeAndNil(vQryRetornoLotes);
    FreeAndNil(vQryRegistrarRetorno);
    FreeAndNil(vQryLacres);
  End;
end;
