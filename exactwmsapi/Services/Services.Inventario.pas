...
Var Query, vQryApuracao: TFDQuery;
    xPedido, vApuracao, pProcessoId: Integer;
begin
  Query        := TFDQuery.Create(nil);
  vQryApuracao := TFDQuery.Create(nil);
  Try
    Query.Connection        := Connection;
    vQryApuracao.Connection := Connection;
    Query.Connection.ResourceOptions.CmdExecTimeout := 120000;
    vQryApuracao.Connection.ResourceOptions.CmdExecTimeout := 120000;
    Try
      ...
      if Query.FieldByName('InventarioTipo').AsInteger = 1 then Begin
         Query.Close;
         Query.SQL.Clear;
         //Veficar se não existe bloqueio em outros Inventarios Abertos
      ...
      End;
      if DebugHook <> 0 then
         Query.SQL.SaveToFile('ApuracaoInventario01.Sql');
      Query.ExecSql;
      Query.Close;
      Query.SQL.Clear;
      ...
      vQryApuracao.SQL.Add('If (Select InventarioTipo From Inventarios Where InventarioId = @InventarioId) = 1 begin --Por Endereco');
      ...
      vQryApuracao.SQL.Add('	          Where II.LoteId Is Null and Est.EstoqueTipoId In (4)');
      ...
      vQryApuracao.SQL.Add('End;');
      if DebugHook <> 0 then
         vQryApuracao.SQL.SaveToFile('ApuracaoInventario02.Sql');
    ...
    Except On E: Exception do
      Begin
        ...
        raise Exception.Create('Processo: ApuraçãoInventarioEndereco - '+TUtil.TratarExcessao(E.Message));
      End;
    End;
  Finally
    FreeAndNil(Query);
    FreeAndNil(vQryApuracao);
  End;
end;
...
function TServiceInventario.Cancelar(const AParams : TDictionary<string, string>): Boolean;
begin
...
begin
  Try
    vQryInventario := TFdQuery.Create(Nil);
    vQryEndereco   := TFdQuery.Create(nil);
    Try
      Result := TJsonObject.Create;
      vQryInventario.connection := Connection;
      vQryEndereco.connection   := Connection;
      vQryInventario.Close;
      vQryInventario.SQL.Add(TuEvolutConst.SqlDSHAcompanhamentoContagemInventario);
      vParamOk := 0;
      if AParams.ContainsKey('inventarioidinicial') then Begin
        vQryInventario.ParamByName('pInventarioidInicial').Value := AParams.Items['inventarioidinicial'];
        Inc(vParamOk);
      end
      Else
    ...
    Except On E: Exception do
      Begin
        raise Exception.Create('Processo: GetDSHAcompanhamentoContagem - '+TUtil.TratarExcessao(E.Message));
      End;
    End;
  Finally
    FreeAndNil(vQryInventario);
    FreeAndNil(vQryEndereco);
  End;
end;
...
function TServiceInventario.GetLoteInventariado(const AParams
  : TDictionary<string, string>): TJsonArray;
Var vQryLoteInvetariado: TFDQuery;
    vParamOk: Integer;
    vProcessoId: Integer;
begin
  Try
    vQryLoteInvetariado := TFdQuery.Create(Nil);
    Try
      vQryLoteInvetariado.Connection := Connection;
      Result := TJsonArray.Create;
    ...
    End;
  Finally
    FreeAndNil(vQryLoteInvetariado);
  End;
end;
