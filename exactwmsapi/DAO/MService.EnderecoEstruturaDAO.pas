...
type
  ...
  public
    ...
    function GetEstrutura4D(const AParams: TDictionary<string, string>) : TJsonObject;
...
begin
  ...
  Try
    try
      vSql := 'Delete from Enderecamentoestruturas where EstruturaId = ' + pId.ToString;
      Query.ExecSQL(vSql);
      Result := True;
  ...
  Try
    ...
    Try
      Query.Open('SELECT COLUMN_NAME As Nome, DATA_TYPE As Tipo, Coalesce(CHARACTER_MAXIMUM_LENGTH, 0) as Tamanho'+sLineBreak+
                 ...
                 'Where TABLE_NAME = ' + QuotedStr('Enderecamentoestruturas') + ' and CHARACTER_MAXIMUM_LENGTH Is Not Null');
...
begin
  ...
  Try
    Query.Connection := Connection;
    try
      vSql := 'select * from Enderecamentoestruturas where Descricao like ' + QuotedStr('%' + pNome + '%');
      Query.Open(vSql);
...
begin
  ...
  Try
    Try
      ...
      QryRecordCount.SQL.Add('Select Count(EstruturaId) cReg From EnderecamentoEstruturas where 1=1');
      if AParams.ContainsKey('estruturaid') then begin
         QryPesquisa.SQL.Add('and EstruturaId = :EstruturaId');
         QryPesquisa.ParamByName('EstruturaId').AsString := AParams.Items['estruturaid'];
         QryRecordCount.SQL.Add('and EstruturaId = :EstruturaId');
         QryRecordCount.ParamByName('EstruturaId').AsString := AParams.Items['estruturaid'];
      end;
...
begin
  ...
  Try
    ...
    try
       ...
       Else
          vSql := 'select * from Enderecamentoestruturas where EstruturaId = ' + pId.ToString;
       vSql := vSql + sLineBreak + 'Order by EstruturaId';
       Query.Open(vSql);
       Result := Query.toJsonArray;
    Except on E: Exception do
       raise Exception.Create(E.Message);
...
function TEnderecoestruturaDao.InsertUpdate(pJsonEstrutura: TJsonObject) : TjSonArray;
var vSql: String;
    Query : TFdQuery;
begin
  Query := TFDQuery.Create(nil);
  Try
    Query.Connection := Connection;
    try
      if pJsonEstrutura.GetValue<Integer>('estruturaid') = 0 then Begin
        vSql := 'Insert Into Enderecamentoestruturas (CodEstrutura, Descricao, PickingFixo, ' + sLineBreak+
                'Distribuicao, Altura, Largura, Comprimento, VolumePadrao, CapacidadePadrao, Mascara, Status) Values ('+sLineBreak+
                '(SELECT IDENT_CURRENT(' + #39 + 'EnderecamentoEstruturas' + #39 +')), ' + QuotedStr(pJsonEstrutura.GetValue<String>('descricao')) + ', '+sLineBreak+
                pJsonEstrutura.GetValue<Integer>('pickingfixo').ToString() + ', ' + pJsonEstrutura.GetValue<Integer>('distribuicao').ToString() + ', '+sLineBreak+
                StringReplace(pJsonEstrutura.GetValue<double>('altura').ToString(), ',','.', [rfReplaceAll]) + ', ' + sLineBreak+
                StringReplace(pJsonEstrutura.GetValue<double>('largua').ToString(), ',','.', [rfReplaceAll]) + ', ' + sLineBreak+
                StringReplace(pJsonEstrutura.GetValue<double>('comprimento').ToString(), ',', '.', [rfReplaceAll]) + ', ' +sLineBreak+
                StringReplace(pJsonEstrutura.GetValue<double>('volumepadrao').ToString(), ',', '.', [rfReplaceAll]) + ', '+sLineBreak+
                StringReplace(pJsonEstrutura.GetValue<double>('capacidadepadrao').ToString(), ',', '.', [rfReplaceAll]) + ', '+sLIneBreak+
                QuotedStr(pJsonEstrutura.GetValue<String>('mascara')) + ', 1)';
      End
      Else
        vSql := 'Update Enderecamentoestruturas '+sLineBreak+
                '   Set Descricao     = ' + QuotedStr(pJsonEstrutura.GetValue<String>('descricao')) + sLineBreak +
          '        , PickingFixo      = ' + pJsonEstrutura.GetValue<Integer>('pickingfixo').ToString() + sLineBreak +
          '        , Distribuicao     = ' + pJsonEstrutura.GetValue<Integer>('distribuicao').ToString() + sLineBreak +
          '        , Altura           = ' + StringReplace(pJsonEstrutura.GetValue<double>('altura').ToString(), ',', '.', [rfReplaceAll])+sLineBreak+
          '        , Largura          = ' + StringReplace(pJsonEstrutura.GetValue<double>('largua').ToString(), ',', '.', [rfReplaceAll])+sLineBreak+
          '        , Comprimento      = ' + StringReplace(pJsonEstrutura.GetValue<double>('comprimento').ToString(), ',', '.', [rfReplaceAll])+sLineBreak+
          '        , VolumePadrao     = ' + StringReplace(pJsonEstrutura.GetValue<double>('volumepadrao').ToString(), ',', '.', [rfReplaceAll]) + sLineBreak +
          '        , CapacidadePadrao = ' +
          StringReplace(pJsonEstrutura.GetValue<double>('capacidadepadrao').ToString(), ',', '.', [rfReplaceAll]) + sLineBreak +
          '        , Mascara          = ' + QuotedStr(pJsonEstrutura.GetValue<String>('mascara')) + sLineBreak +
          '        , Status           = ' + pJsonEstrutura.GetValue<Integer>('status').ToString()+sLineBreak+
          'where EstruturaId = ' + pJsonEstrutura.GetValue<Integer>('estruturaid').ToString();
      Query.ExecSQL(vSql);
      Result := Query.toJsonArray;
    Except On E: Exception do
      raise Exception.Create(E.Message);
