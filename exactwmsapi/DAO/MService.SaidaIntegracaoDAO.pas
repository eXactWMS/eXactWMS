...
type
  ...
  private
    ...
    Procedure SalvarDestinatario(pCodigo: Integer = 0; pRazao: String = '';
      ...
      pHomePage: String = ''; FConexao : TConnectionPool = Nil);
    Procedure SalvarProduto(pCodProduto: Integer = 0; pDescricao: String = '';
      ...
      pEan: String = ''; FConexao: TConnectionPool = Nil);
    ...
    Function SalvarSaida(pPedidoId: Integer = 0; pPessoaId: Integer = 0;
      ...
      FConexao: TConnectionPool = Nil): Integer;
    Procedure SalvarSaidaItens(pPedidoId, pCodProduto: Integer;
      pQuantidade: Integer; pEmbalagemPadrao: Integer; FConexao: TConnectionPool);
  public
...
begin
  ...
  Try
    ...
    try
      Query.Sql.Add(TuEvolutConst.jsonSaidaRetornoCheckInFinalizado);
      if DebugHook <> 0 then
         Query.Sql.SaveToFile('ConsultaSaidaRetorno.Sql');
      Query.Open();
      if Query.FieldByName('ConsultaRetorno').AsString = '' then Begin
    ...
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: SaidaIntegracaoConsulta - ' + TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    Query.Free;
...
begin
  ...
  Try
    ...
    try
      ...
      if DebugHook <> 0 then
         Query.Sql.SaveToFile('ConsultaSaidaRetorno2.Sql');
       Query.Open();
      if Query.FieldByName('ConsultaRetorno').AsString = '' then Begin
         Result := TJsonArray.Create;
         Result.AddElement(TJsonObject.Create.AddPair('Erro', TuEvolutConst.QrySemDados));
...
begin
  Result := TJsonArray.Create;
  vQryJson        := TFDQuery.Create(nil);
  vQryTransaction := TFDQuery.Create(nil);
  Try
    vQryJson.Connection        := Connection;
    vQryTransaction.Connection := Connection;
    Try
      Try
        vQryJson.Sql.Add('Insert Into JsonIntegracao Values (NewId(), ' + #39 + ArraySaidas.toString() + #39 + ')');
        If DebugHook <> 0 Then
      ...
      Try
        Try
          ...
          SalvarDestinatario(jsonDestinatario.GetValue<Integer>('destinatarioid'),
                             ...
                             jsonDestinatario.GetValue<String>('email'), '', Nil);
          ...
          Begin
            ...
            Begin
              ...
              SalvarProduto(JsonItens.Get(xItens).GetValue<Integer>('produtoid', 0),
                ...
                0, 0, 8, 8, 8, 0, 0, JsonItens.Get(xItens).GetValue<String>('ean'), Nil);
              ErroJson := 'SalvarProdutoCodBarras';
              if StrToInt64Def(JsonItens.Get(xItens).GetValue<String>('ean'), 0) > 999999 then
                 SalvarProdutoCodbarras(JsonItens.Get(xItens).GetValue<Integer>('produtoid', 0),
                                        JsonItens.Get(xItens).GetValue<String>('ean'), Nil);
              ...
              SalvarSaidaItens(vSaidaId, JsonItens.Get(xItens)
                ...
                .GetValue<Integer>('quantidade', 0), vEmbalagemPadrao, Nil);
            End;
        ...
        Except On E: Exception do
          Begin
            vQryTransaction.connection.Rollback;
            raise Exception.Create('Processo: SaidaIntegracaos: ' +TUtil.TratarExcessao(E.Message));
          End;
        end;
      Finally
        vQryTransaction.Free;
      End;
    End;
  Finally
    vQryJson.Free;
...
begin
  ...
  Try
    ...
    Try
      QueryR.Sql.Add('Update Pedido Set Status = 5 Where PedidoId = ' + pSaidaId.toString());
      QueryR.ExecSQL;
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: SaidaIntegracao - RegistrarRetorno: ' + TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    QueryR.Free;
...
begin
  ...
  vQryPedVolumes  := TFdQuery.Create(nil);
  vQryPedidoLotes := TFdQuery.Create(nil);
  Try
    VQryPed.Connection         := Connection;
    vQryPedVolumes.Connection  := Connection;
    vQryPedidoLotes.Connection := Connection;
    try
      ...
      With VQryPed do
        While Not Eof do Begin
          ...
          jsonDestinatario.AddPair('destinatarioid', tjsonNumber.Create(VQryPed.FieldByName('DestinatarioId').AsInteger));
          ...
          While Not vQryPedVolumes.Eof do begin
            ...
            jsonPedidoVolume.AddPair('pedidovolumeid', tjsonNumber.Create(vQryPedVolumes.FieldByName('PedidoVolumeId').AsInteger));
            jsonPedidoVolume.AddPair('sequencia', tjsonNumber.Create(vQryPedVolumes.FieldByName('Sequencia').AsInteger));
            jsonPedidoVolume.AddPair('caixa', tjsonNumber.Create(vQryPedVolumes.FieldByName('EmbalagemId').AsInteger));
            jsonPedidoVolume.AddPair('lacres', '[]'); // tjsonPedidoVolumeLacres);
            vQryPedidoLotes.Close;
            vQryPedidoLotes.Sql.Clear;
            vQryPedidoLotes.Sql.Add(TuEvolutConst.SqlSaidaIntegracaoRetornoVolumeLotes);
            vQryPedidoLotes.ParamByName('pPedidoVolumeId').Value := vQryPedVolumes.FieldByName('PedidoVolumeId').AsInteger;
            vQryPedidoLotes.Open;
            ArrayJsonProdutos := TJsonArray.Create;
            While Not vQryPedidoLotes.Eof do begin
              jsonPedidoProduto := TJsonObject.Create;
              jsonPedidoProduto.AddPair('produtoid', tjsonNumber.Create(vQryPedidoLotes.FieldByName('CodProduto').AsInteger));
              jsonPedidoProduto.AddPair('ean', vQryPedidoLotes.FieldByName('CodBarras').AsString);
              ...
              while (not vQryPedidoLotes.Eof) and (vCodProduto = vQryPedidoLotes.FieldByName('CodProduto').AsInteger) do Begin
                ...
                jsonPedidoLotes.AddPair('loteid', tjsonNumber.Create(vQryPedidoLotes.FieldByName('LoteId').AsInteger));
              ...
              jsonPedidoProduto.AddPair('itemid', tjsonNumber.Create(vQryPedidoLotes.FieldByName('ItemIdERP').AsInteger));
    ...
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: SaidaIntegracaosRetorno - ' +TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    VQryPed.Free;
    vQryPedVolumes.Free;
    vQryPedidoLotes.Free;
  End;
end;
...
Function TSaidaIntegracaoDao.SalvarSaida(pPedidoId, pPessoaId: Integer;
  ...
  FConexao: TConnectionPool): Integer;
Var vQry: TFDQuery;
...
begin
  ...
  Try
    ...
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: SalvarSaida(Integracao) - '+TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    vQry.Free;
...
begin
  ...
  Try
    ...
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: SalvarSaidaItens(Integracaos): ' + TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
   vQryItens.Free;
...
Procedure TSaidaIntegracaoDao.SalvarDestinatario(pCodigo: Integer;
  pRazao, pFantasia, pCnpj, pEmail, pHomePage: String; FConexao: TConnectionPool);
Var vQry: TFDQuery;
begin
  vQry := TFdQuery.Create(Nil);
  Try
    ...
    Try
      ...
      vQry.ParamByName('pCnpjCpf').Value := StringReplace(StringReplace(StringReplace(pCnpj, '.', '', [rfReplaceAll]), '-', '', [rfReplaceAll]), '/', '', [rfReplaceAll]);
    ...
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: SaidaIntegracao - Destinatario: ' + TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    vQry.Free;
...
begin
  ...
  Try
    ...
    Except ON E: Exception do Begin
        raise Exception.Create('Processo: SaidaIntegracaos - Produtos: ' + StringReplace(E.Message, '[FireDAC][Phys][ODBC][Microsoft][SQL Server Native Client 11.0][SQL Server]', '', [rfReplaceAll]));
      End;
    end;
  Finally
    vQry.Free;
...
procedure TSaidaIntegracaoDao.SalvarProdutoCodbarras(pCodProdutoERP: Integer;
  pEan: String; FConexao: TConnectionPool);
Var vQry: TFDQuery;
begin
  vQry := TFdQuery.Create(nil);
  Try
    ...
    Except ON E: Exception do
      Begin
        raise Exception.Create('Processo: SaidaIntegracao - Código de Barras(' + TUtil.TratarExcessao(E.Message));
      End;
    end;
  Finally
    vQry.Free;
