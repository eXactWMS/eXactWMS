  ...
  Try
    ...
    Try
      ...
      Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
                      'v1/lote', Trim(Req.Params.Content.Text), Req.Body, '', 'Retorno: ' + jsonArrayRetorno.Count.ToString + ' Registros.',
                      200, calculaTempoProcesso(HrInicioLog), Req.Headers['appname']+'_V: '+Req.Headers['versao']);
    Except On E: Exception do
      Begin
        ...
        Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
                        'v1/lote', Trim(Req.Params.Content.Text), Req.Body, '', JsonArrayErro.ToString(),
                        500, calculaTempoProcesso(HrInicioLog), Req.Headers['appname']+'_V: '+Req.Headers['versao']);
      End;
    End;
  Finally
...
begin
  Try
    ...
    try
      ...
      Res.Send<TJSonArray>(JsonArrayRetorno).Status(THttpStatus.Created);
      Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
                      'v1/lote/:produtoid/:loteid/:descricao', Trim(Req.Params.Content.Text), Req.Body, '', 'Retorno: ' + jsonArrayRetorno.Count.ToString + ' Registros.',
                      200, calculaTempoProcesso(HrInicioLog), Req.Headers['appname']+'_V: '+Req.Headers['versao']);
    Except
      On E: Exception do
      Begin
        ...
        Tutil.SalvarLog(Req.MethodType,
          StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'],
          ClientIP(Req), THorse.Port, 'v1/lote/:produtoid/:loteid/:descricao',
  ...
  Try
    ...
    try
      ...
      if StrToIntDef(Req.Params.Items['agrupado'], 0) = 0 then
         JsonArrayRetorno := LoteDAO.GetLoteResumo(Req.Params.Items['produtoid'].ToInteger(), Req.Params.Items['descricao'])
      Else
        JsonArrayRetorno := LoteDAO.GetLoteResumoAgrupado(Req.Params.Items['produtoid'].ToInteger(), Req.Params.Items['descricao']);
      Res.Send<TJSonArray>(JsonArrayRetorno).Status(THttpStatus.Created);
      Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
                      'v1/lote/resumo/:produtoid/:descricao/:agrupado', Trim(Req.Params.Content.Text), Req.Body, '', 'Retorno: ' + jsonArrayRetorno.Count.ToString + ' Registros.',
                      200, calculaTempoProcesso(HrInicioLog), Req.Headers['appname']+'_V: '+Req.Headers['versao']);

    Except On E: Exception do
      Begin
        JsonArrayErro := TJSonArray.Create;
        JsonArrayErro.AddElement(TJSONObject.Create(TJSONPair.Create('Resultado', E.Message)));
        Res.Send<TJSonArray>(JsonArrayErro).Status(THttpStatus.ExpectationFailed);
        Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
                        'v1/lote/resumo/:produtoid/:descricao/:agrupado', Trim(Req.Params.Content.Text), Req.Body, '', JsonArrayErro.ToString(),
                        500, calculaTempoProcesso(HrInicioLog), Req.Headers['appname']+'_V: '+Req.Headers['versao']);
      End;
    end;
  Finally
