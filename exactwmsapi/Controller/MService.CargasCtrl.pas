...
Procedure GetCargaPedidoVolumesConferencia(Req: THorseRequest; Res: THorseResponse; Next: TProc);
...
begin
  ...
  // .Get('/cargas/volumes/:cargaid/:processo', GetCargaPedidoVolumes)     //
    ...
    .Get('/cargas/volumes/conferencia/:cargaid/:processo', GetCargaPedidoVolumesConferencia)
...
begin
  Try
    Try
      ...
      Res.Send<TJSONObject>(ObjCargasDAO.Get4D(Req.Query.Dictionary)).Status(THTTPStatus.OK);
...
begin
  Try
    ...
    Try
      ObjCargaDAO      := TCargasDao.Create;
      JsonArrayRetorno := ObjCargaDAO.GetCargaPedidoVolumesConferencia(StrToIntDef(Req.Params.Items['cargaid'], 0), Req.Params.Items['processo']);
      Res.Send<TJsonArray>(JsonArrayRetorno).Status(THTTPStatus.Ok);
      Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
                      '/v1/cargas/volumes/conferencia/:cargaid/:processo', Trim(Req.Params.Content.Text), Req.Body, '', 'Retorno: ' + JsonArrayRetorno.Count.ToString + ' Registros.',
                      201, calculaTempoProcesso(HrInicioLog), Req.Headers['appname']+'_V: '+Req.Headers['versao']);
    Except on E: Exception do
      Begin
        JsonArrayRetorno := TJsonArray.Create;
        ...
        Res.Send<TJsonArray>(JsonArrayRetorno).Status(THTTPStatus.InternalServerError);
        Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
                        '/v1/cargas/volumes/conferencia/:cargaid/:processo', Trim(Req.Params.Content.Text), Req.Body, '', E.Message,
                        500, calculaTempoProcesso(HrInicioLog), Req.Headers['appname']+'_V: '+Req.Headers['versao']);
      End;
    End;
  Finally
...
begin
  Try
    ...
    Try
      ObjCargaDAO      := TCargasDao.Create;
      JsonArrayRetorno := ObjCargaDAO.GetCargaPedidoVolumes(StrToIntDef(Req.Params.Items['cargaid'], 0), Req.Params.Items['processo']);
      Res.Send<TJsonArray>(JsonArrayRetorno).Status(THTTPStatus.Ok);
      Tutil.SalvarLog(Req.MethodType, StrToIntDef(Req.Headers['usuarioid'], 0), Req.Headers['terminal'], ClientIP(Req), THorse.Port,
