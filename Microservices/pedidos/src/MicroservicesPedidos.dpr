program MicroservicesPedidos;

{$APPTYPE CONSOLE}
{$R *.res}

uses
  System.SysUtils,
  RabbitConsumer in 'exactwms.pedidos.common\RabbitConsumer.pas',
  PedidosController in 'exactwms.pedidos.controller\PedidosController.pas',
  GerarPedido in 'exactwms.pedidos.services\GerarPedido.pas',
  GerarPedidoCaixaFechada
    in 'exactwms.pedidos.services\GerarPedidoCaixaFechada.pas',
  GerarPedidoCaixaFracionada
    in 'exactwms.pedidos.services\GerarPedidoCaixaFracionada.pas',
  PedidoSaidaCtrl in '..\..\..\Controller\PedidoSaidaCtrl.pas',
  PedidoProdutoCtrl in '..\..\..\Controller\PedidoProdutoCtrl.pas',
  PedidoSaidaClass in '..\..\..\Model\PedidoSaidaClass.pas',
  PedidoProdutoClass in '..\..\..\Model\PedidoProdutoClass.pas',
  ProdutoClass in '..\..\..\Model\ProdutoClass.pas',
  LaboratoriosClass in '..\..\..\Model\LaboratoriosClass.Pas',
  RastroClass in '..\..\..\Model\RastroClass.pas',
  PedidoClass in '..\..\..\Model\PedidoClass.pas',
  PessoaClass in '..\..\..\Model\PessoaClass.Pas',
  PessoaTipoClass in '..\..\..\Model\PessoaTipoClass.pas',
  OperacaoTipoClass in '..\..\..\Model\OperacaoTipoClass.Pas',
  OperacaoNaturezaClass in '..\..\..\Model\OperacaoNaturezaClass.Pas',
  MedicamentoTipoClass in '..\..\..\Model\MedicamentoTipoClass.pas',
  ProdutoTipoClass in '..\..\..\Model\ProdutoTipoClass.pas',
  UnidadeClass in '..\..\..\Model\UnidadeClass.pas',
  EnderecoClass in '..\..\..\Model\EnderecoClass.pas',
  LotesClass in '..\..\..\Model\LotesClass.pas',
  EnderecamentoRuaClass in '..\..\..\Model\EnderecamentoRuaClass.pas',
  EnderecamentoZonaClass in '..\..\..\Model\EnderecamentoZonaClass.pas',
  EnderecoEstruturaClass in '..\..\..\Model\EnderecoEstruturaClass.pas',
  DesenhoArmazemClass in '..\..\..\Model\DesenhoArmazemClass.pas',
  uDmBase in 'exactwms.pedidos.dao\uDmBase.pas' {DmBase: TDataModule} ,
  uDmeXactWMS
    in 'exactwms.pedidos.dao\uDmeXactWMS.pas' {DmeXactWMS: TDataModule} ,
  ConfiguracaoClass in 'exactwms.pedidos.common\ConfiguracaoClass.pas',
  uLaboratorioDAO in '..\..\..\DAORESTFULL\uLaboratorioDAO.pas',
  uFuncoes in 'exactwms.pedidos.common\uFuncoes.pas',
  uPedidoSaidaDAO in '..\..\..\DAORESTFULL\uPedidoSaidaDAO.pas',
  EstoqueCaixaFechada in 'exactwms.pedidos.services\EstoqueCaixaFechada.pas',
  PedidoVolumeCtrl in 'exactwms.pedidos.controller\PedidoVolumeCtrl.pas',
  PedidoVolumeClass in '..\..\..\Model\PedidoVolumeClass.pas',
  EmbalagemCaixaClass in '..\..\..\Model\EmbalagemCaixaClass.pas',
  UsuarioClass in '..\..\..\Model\UsuarioClass.pas',
  PerfilClass in '..\..\..\Model\PerfilClass.pas',
  VolumeEmbalagemClass in '..\..\..\Model\VolumeEmbalagemClass.pas',
  PedidoVolumeDAO in '..\..\..\DAORESTFULL\PedidoVolumeDAO.pas',
  uFrmeXactWMS in 'exactwms.pedidos.common\uFrmeXactWMS.pas',
  VolumeEmbalagemCtrl in '..\..\..\Controller\VolumeEmbalagemCtrl.pas',
  uVolumeEmbalagemDAO in '..\..\..\DAORESTFULL\uVolumeEmbalagemDAO.pas';

var
  fila: string;
   strlog:string;
begin
  try

    strlog :=
 '-------------------------------------------------------------------------------------'+ #13#10+
 '-------------------------------------------------------------------------------------'+ #13#10+
 ''  + #13#10+
 '/ ___|| ____|  _ \ \   / /_ _/ ___| ____|  |  _ \| ____|  _ \_ _|  _ \ / _ \/ ___|   '+  #13#10+
 '\___ \|  _| | |_) \ \ / / | | |   |  _|    | |_) |  _| | | | | || | | | | | \___ \   '+  #13#10+
 ' ___) | |___|  _ < \ V /  | | |___| |___   |  __/| |___| |_| | || |_| | |_| |___) |  '+  #13#10+
 '|____/|_____|_| \_\ \_/  |___\____|_____|  |_|   |_____|____/___|____/ \___/|____/   '+  #13#10+
 ''  + #13#10+
 '--------------------------------------------------------------------------------------'+ #13#10+
 '                            v 1.0.1'  + #13#10+
 '--------------------------------------------------------------------------------------';
    Writeln(strlog);
     Writeln('Iniciando worker...');
    // implementar aqqui de que fila vai ler atravez de um ini ou leitura de um banco
    fila:=GetEnvironmentVariable('RABBIT_FILA');
    if fila = '' then
      fila := 'Zona Picking Conveniencia';
    // podem ser lidas varias filas ao mesmo tempo
    Writeln('worker iniciado fila > ' + fila);
    gravalog('Carregando configuracoes...');
    DmeXactWMS := TDmeXactWMS.create(Nil);
    gravalog('Configuracoes carregadas');
    uFrmeXactWMS.FrmeXactWMS := TFrmeXactWMS.create();
    FRabitmqObj := TRabitmqObj.create;
    gravalog('Iniciando comunicacao com mensageria');
    FRabitmqObj.Start();
    FRabitmqObj.consumirFila(fila, TPedidosController.ProcessarPedido);
    while True do
      Sleep(1000);
  except
    on E: Exception do
      gravalog(E.Message);
  end;

end.
