...
uses
  ...
  uFrmCheckListEstoque, FMX.Memo.Types, uFrmCaixaLiberacao;
...
//      Shift: TShiftState);
  ...
  private
    ...
    FCaixaLiberacao : TFrmCaixaLiberacao;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Carga - Conferência')) then Begin
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Check-Out')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Estoque - CheckList')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Caixas - Liberação')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
  if Not Assigned(FCaixaLiberacao) then
     FCaixaLiberacao := TFrmCaixaLiberacao.Create(Self);
  MultiView1.HideMaster;
  Self.LytMain.RemoveObject(0);
  Self.LytMain.AddObject(FCaixaLiberacao.LytBase);
  FCaixaLiberacao.LblTituloForm.Text := 'Caixas - Liberação';
  ShowForm(FCaixaLiberacao.LytBase);
  FCaixaLiberacao.PgcPrincipal.ActiveTab := FCaixaLiberacao.TabDetalhes;
  DelayedSetFocus(FCaixaLiberacao.EdtCodigo);
  KsLoadingIndicator1.HideLoading;
end;
...
Var I : Integer;
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Carga - Carregar')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Estoque - Consulta')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Expedição - Registrar Volume')) then Begin //Registrar Volume na Expedição
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Inventário - Contagem')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Armazenagem')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Produtos')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
//  FSeparacao.ShowLoading;
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Apanhe - Separação')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Endereçar Produto em Picking')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
  KsLoadingIndicator1.ShowLoading;
...
begin
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Recebimentos')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
...
begin
//  KsLoadingIndicator1.ShowLoading;
  if Not (FrmeXactWMS.ObjUsuarioCtrl.AcessoFuncionalidade('Reposição de picking')) then Begin
     MensToast('Usuário não autorizado!', CorToastAlerta, 'alarme');
     Exit;
  End;
