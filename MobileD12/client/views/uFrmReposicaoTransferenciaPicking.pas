...
type
  ...
  private
    ...
    Procedure AddProdutoListview(pCodProduto, pDescricao, pLote, pVencimento : String; pQtde : Integer);
  protected
...
procedure TFrmReposicaoTransferenciaPicking.AddProdutoListview(pCodProduto,
  pDescricao, pLote, pVencimento: String; pQtde: Integer);
var
    item: TListViewItem;
begin
  item := LstTransferenciaEstoque.Items.Add;
  with item do
  begin
    Tag := pCodproduto.ToInt64();
    TListItemText(Objects.FindDrawable('CodProduto')).Text := pCodProduto;
    TListItemText(Objects.FindDrawable('Descricao')).Text := pDescricao;
    TListItemText(Objects.FindDrawable('Lote')).Text := pLote;
    TListItemText(Objects.FindDrawable('Vencimento')).Text := pVencimento;
    TListItemText(Objects.FindDrawable('Descricao')).Text := pDescricao;
    if pVisualizarSaldoTransferecia then
       TListItemText(Objects.FindDrawable('Saldo')).Text := pQtde.ToString()
    Else
...
begin
  ...
  Try
    Try
      ...
      End
      Else If (FdMemReposicaoTransferencia.FieldByName('BloqueioInventarioEndereco').AsInteger=1) or
              (FdMemReposicaoTransferencia.FieldByName('BloqueioInventarioProduto').AsInteger=1) then Begin
         SetCampoDefault('EdtCodProdutoTransferencia');
         If (FdMemReposicaoTransferencia.FieldByName('BloqueioInventarioEndereco').AsInteger=1) then
            ShowErro('Endereço de Picking Bloqueado para inventariar!')
         Else
            ShowErro('Produto Bloqueado para inventariar!');
      End
      Else Begin
...
begin
  Showloading;
  Try
    JsonArrayRetorno := ObjReposicaoCtrl.GetConsultaReposicao(0, 0, 0, 0, 1, 0);
...
procedure TFrmReposicaoTransferenciaPicking.LstPrincipalGesture(Sender: TObject;
  const EventInfo: TGestureEventInfo; var Handled: Boolean);
begin
//  inherited;
...
begin
    ...
    begin
      ...
      FDMemReposicaoTransferencia.First;
      While Not FDMemReposicaoTransferencia.Eof do begin
        AddProdutoListview(FdMemReposicaoTransferencia.FieldByName('CodProduto').AsString,
                           FdMemReposicaoTransferencia.FieldByName('Descricao').AsString,
                           FdMemReposicaoTransferencia.FieldByName('Lote').AsString,
                           FdMemReposicaoTransferencia.FieldByName('Vencimento').AsString,
                           FdMemReposicaoTransferencia.FieldByName('Qtde').AsInteger);
        FDMemReposicaoTransferencia.Next;
      end;
      TThread.Synchronize(TThread.CurrentThread, procedure
      begin
    ...
    t.OnTerminate := ThreadTerminatePTransf;
    t.Start;
end;
...
begin
  ...
  JsonArrayRetorno := ObjPedidoCtrl.GetConsultaReposicao(0, 0, pReposicaoId, 0, 0, 0);
...
//  JsonTransferecia.AddPair('hora', FdMemReposicaoTransferencia.FieldByName('hrColeta').AsString);
  ...
  Try 
    JsonArrayRetorno := Nil;
    FreeAndNil(JsonTransferecia);
    FreeAndNil(ObjReposicaoCtrl);
  Except 
  End;
end;
...
begin
  ...
  Else if CampoDefault = 'EdtQtdApanhe' then Begin
     ...
     DelayEdSetFocus(EdtQtdApanhe);
  End
  Else if CampoDefault = 'CbLoteTransferecia' then Begin
     CbLoteTransferencia.ItemIndex := -1;
     DelayEdSetFocus(CbLoteTransferencia);
  End
end;
