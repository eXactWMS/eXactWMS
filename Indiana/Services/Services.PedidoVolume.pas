...
begin
  try
    ...
    try
      ...
      vQry.Sql.Add('         (@PedidoVolumeId = 0 or @PedidoVolumeId = PedidoVolumeId) and');
      vQry.Sql.Add('         De.ProcessoId = (Select Max(ProcessoId) From DocumentoEtapas where documento = pv.uuid) And De.ProcessoId <> 15');

      vQry.Sql.Add('Set @ProcessoIdMinVolumeId = (select Coalesce(Min(DE.ProcessoId), 15)');
      vQry.Sql.Add('        From PedidoVolumes PV');
      vQry.Sql.Add('        Inner Join Pedido Ped On Ped.PedidoId = Pv.PedidoId');
      vQry.Sql.Add('        Left Join vDocumentoEtapas DE On De.Documento = Pv.uuid and');
      vQry.Sql.Add('                                         De.ProcessoId = (Select MAX(ProcessoId) From vDocumentoEtapas Where Documento = De.Documento )');
      vQry.Sql.Add('        where Pv.PedidoId = @PedidoId)');
...
//      if DebugHook <> 0 then
    ...
    Except ON E: Exception do
      Begin
        if vTransacao then
          vQry.connection.Rollback;
        raise Exception.Create('Processo: Volumes Cancelar - ' + StringReplace(E.Message,
...
begin
  ...
  Except ON E: Exception do
    Begin
      vQry.Close;
      vQry.connection.Rollback;
      raise Exception.Create('Processo: FinalizarSeparacao - ' + StringReplace(E.Message, '[FireDAC][Phys][ODBC][Microsoft][SQL Server Native Client 11.0][SQL Server]', '', [rfReplaceAll]));
    End;
  end;
end;
...
begin
  ...
  Except ON E: Exception do
    Begin
      vQry.connection.Rollback;
      raise Exception.Create(StringReplace(E.Message, '[FireDAC][Phys][ODBC][Microsoft][SQL Server Native Client 11.0][SQL Server]',
            '', [rfReplaceAll]));
    End;
  end;
end;
...
{      While Not vQry.Eof do Begin
        ...
        JsonVolume.AddPair('pedidoid', TJsonNumber.Create(vQry.FieldByName('PedidoId').AsInteger));
        JsonVolume.AddPair('pedidovolumeid', TJsonNumber.Create(vQry.FieldByName('PedidoVolumeId').AsInteger));
        JsonVolume.AddPair('volumetipo', TJsonNumber.Create(vQry.FieldByName('VolumeTipo').AsInteger));
        JsonVolume.AddPair('embalagem', vQry.FieldByName('Embalagem').AsString);
        JsonVolume.AddPair('processo', vQry.FieldByName('Processo').AsString);
        JsonVolume.AddPair('razao', vQry.FieldByName('Razao').AsString);
        JsonVolume.AddPair('documentodata', vQry.FieldByName('data').AsString);
