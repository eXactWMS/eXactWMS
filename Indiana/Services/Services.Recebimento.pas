...
Const SqlEntrada =
      ...
      '	   Ped.DtInclusao, Ped.HrInclusao, ArmazemId, Ped.Status, PN.agrupamentoid, Ped.Uuid Into #Entrada'+sLineBreak+
...
//      'Group by Pi.PedidoId'+sLineBreak+
      ...
      '	      CONVERT(VARCHAR, RH.Hora ,8) as HrInclusao, Ent.ArmazemId, Ent.Status, Mp.MaxProcessoId ProcessoId, Pe.Descricao Processo,'+sLineBreaK+
      ...
      'Inner Join ProcessoEtapas PE On Pe.ProcessoId = Mp.MaxProcessoId'+sLineBreak+
...
begin
  ...
  Except On E: Exception do
    Begin
      raise Exception.Create('Processo: DeleteAgrupamento - ' + StringReplace(E.Message,
...
begin
  Try
    ...
    FConexao.Query.SQL.Add('  Where @Agrupamentoid = PAC.AgrupamentoId and Pl.IdProduto = @ProdutoId');
    FConexao.Query.SQL.Add('End;');
    FConexao.Query.ParamByName('pAgrupamentoId').Value := pAgrupamentoId;
    FConexao.Query.ParamByName('pPedidoId').Value      := pPedidoId;
...
begin
  ...
  Except On E: Exception do
    Begin
      ...
      vQryFinalizar.Close;
      vQryKardex.Close;
      raise Exception.Create('Processo: FinalizarCheckInService - ' + StringReplace(E.Message,
                             '[FireDAC][Phys][ODBC][Microsoft][SQL Server Native Client 11.0][SQL Server]', '', [rfReplaceAll]));
    End;
  End;
end;
...
begin
  Try
    FConexao.Query.Sql.Add('Select Pac.*, Pl.IdProduto ProdutoId, Pl.Lote DescrLote, Pl.Data Fabricacao,');
    FConexao.Query.Sql.Add('       Pl.Vencimento, Rd.Data, Rh.Hora');
    FConexao.Query.Sql.Add('from PedidoItensCheckInAgrupamento Pac');
    FConexao.Query.Sql.Add('Inner join vProdutoLotes Pl on Pl.Loteid = Pac.Loteid');
    ...
    FConexao.Query.SQL.Add('Where AgrupamentoId = :pAgrupamentoId');
    FConexao.Query.SQL.Add('Order By Pl.IdProduto, Pl.LoteId');
    FConexao.Query.ParamByName('pAgrupamentoId').Value := pAgrupamentoId;
    if DebugHook <> 0 then
...
begin
  ...
  Except On E: Exception do
    Begin
      raise Exception.Create('Processo: GetAgrupamentoFatorarLoteXML - ' + StringReplace(E.Message,
            '[FireDAC][Phys][ODBC][Microsoft][SQL Server Native Client 11.0][SQL Server]', '', [rfReplaceAll]));
    End;
  End;
end;
...
begin
  ...
  Except On E: Exception do
    Begin
      FConexao.Query.connection.Rollback;
      raise Exception.Create('Processo: SalvarCheckInItemAgrupamento - ' + StringReplace(E.Message,
            '[FireDAC][Phys][ODBC][Microsoft][SQL Server Native Client 11.0][SQL Server]', '', [rfReplaceAll]));
    End;
  End;
end;
