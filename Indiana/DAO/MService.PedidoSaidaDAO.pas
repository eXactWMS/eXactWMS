  ...
  try
    ...
    while Not FConexao.Query.Eof do Begin
      ...
      Except On E: Exception do
          raise Exception.Create('Processo: CancelarVolumes - ' + StringReplace(E.Message,
...
begin
  Result := 'declare @DtInicio DateTime  = :pDtInicio' +sLineBreak+
            ...
            'Inner join (Select Distinct PedidoVolumeId, Pl.ZonaId'+sLineBreak+
  ...
  if pZonaIdStr <> '' then
     Result := Result + sLineBreak + '			Where Pl.Zonaid in ('+pZonaIdStr+')';
  Result := Result + '			) Vl On Vl.PedidoVolumeId = Pv.PedidoVolumeId'+sLineBreak+
            ...
            ''+sLineBreak+
            'Select IsNull(ZonaId, 0) Zonaid, IsNull(Zona, '+#39+'SEM PICKING'+#39+') Zona,'+sLineBreak+
...
begin
  Result := 'declare @DtInicio DateTime = :pDtInicio'+sLineBreak+
            ...
            'Left Join RotaPessoas Rp On Rp.pessoaid = Pes.PessoaId'+sLineBreak+
            'inner Join PedidoVolumes Pv On Pv.PedidoId = Ped.PedidoId'+sLineBreak+
            'Inner join (Select Distinct PedidoVolumeId, ZonaId'+sLineBreak+
            '            From PedidoVolumeLotes Vl'+sLineBreak+
            '			       Inner join vProdutolotes Pl On Pl.LoteId = Vl.LoteId';
  If pZonaIdStr <> '' then
     Result := Result+sLineBreak+'			Where Pl.Zonaid in ('+pZonaIdStr+')';
  Result := Result+sLineBreak+'			) Vl On Vl.PedidoVolumeId = Pv.PedidoVolumeId'+sLineBreak+
            'Left Join vDocumentoEtapas DE On De.Documento = Pv.uuid'+sLineBreak+
            'Left Join Rhema_data Rd On Rd.IdData = Ped.documentoData'+sLineBreak+
            'Left Join EnderecamentoZonas Z On Z.ZonaId = Vl.ZonaID'+sLineBreak+
...
begin
  Result := 'declare @DtInicio DateTime = :pDtInicio'+sLineBreak+
            ...
            ''+sLineBreak+
            'Select PedidoId, IsNull(ZonaId, 0) ZonaId, Quantidade Into #ProdZonas'+sLineBreak+
